...
- **Tier 驗證**：腳本 tier_v2 = 🟢 Back-end Starter（xFIP p26, K-BB% p27），gap vs ERA-only = -26.4
...
- **Tier 驗證**：腳本 tier_v2 = 🟠 Strong Ace（xFIP p95, K-BB% p64），gap vs ERA-only = +35.7
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 4.65 / 4 / <!-- AI --> | 3.69 / 8 / <!-- AI --> |
...
- ⚠️ HOME 打線 Flag 3 (last7 BABIP=0.245):
  - <!-- AI 補：可能回歸或可能持續？是否影響本場判斷？不自動 ±run value -->

### 額外信號
- 🔴 HOME reverse platoon Δ +0.361（vs RHB OPS 0.915 > vs LHB OPS 0.554）— RHP 對非預期手別反而吃虧
- 🟠 HOME TTO3 penalty：OPS Δ +-0.044（TTO1 0.706 → TTO3 0.662），第三輪明顯衰退；K% 從 18.1% 掉到 9.1%（Δ -9.0pp）
- 🟠 HOME chain breaks at #6-7：OPS 落差 0.241
- 🟠 AWAY chain breaks at #6-7：OPS 落差 0.265
...
- 天氣：Partly Cloudy, 80°F, wind 10 mph, Out To RF
