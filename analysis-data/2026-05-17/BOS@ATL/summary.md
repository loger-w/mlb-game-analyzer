...
- **Tier 驗證**：腳本 tier_v2 = 🟢 Back-end Starter（xFIP p36, K-BB% p30），gap vs ERA-only = -6.0
...
### Brayan Bello (AWAY, RHP, 27 ⚡ 巔峰期)
- **Tier 驗證**：腳本 tier_v2 = 🟡 Solid Starter（xFIP p55, K-BB% p25），gap vs ERA-only = +39.2
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 3.04 / 7 / <!-- AI --> | 3.16 / 6 / <!-- AI --> |
...
### 額外信號
- 🟠 HOME TTO3 penalty：OPS Δ +-0.012（TTO1 0.595 → TTO3 0.583），第三輪明顯衰退；K% 從 21.2% 掉到 15.2%（Δ -6.0pp）
- 🔴 AWAY TTO3 penalty：OPS Δ +0.207（TTO1 0.834 → TTO3 1.041），第三輪明顯衰退；K% 從 23.6% 掉到 18.4%（Δ -5.2pp）
- 🟠 HOME chain breaks at #7-8：OPS 落差 0.203
- 🔴 AWAY chain breaks at #8-9：OPS 落差 0.629
...
- 天氣：Partly Cloudy, 83°F, wind 8 mph, L To R
...
| HOME | 6.4 | <!-- AI 補 --> | <!-- AI 補 --> |
| AWAY | 5.7 | <!-- AI 補 --> | <!-- AI 補 --> |
| Total | 12.100000000000001 | <!-- AI 補 --> | <!-- AI 補 --> |
