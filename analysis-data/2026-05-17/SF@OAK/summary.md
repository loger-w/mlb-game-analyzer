...
- **Tier 驗證**：腳本 tier_v2 = 🟠 Strong Ace（xFIP p66, K-BB% p74），gap vs ERA-only = +17.0
...
### Adrian Houser (AWAY, RHP, 33 📉 初期退化)
- **Tier 驗證**：腳本 tier_v2 = 🟢 Back-end Starter（xFIP p37, K-BB% p17），gap vs ERA-only = +19.4
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 4.49 / 2 / <!-- AI --> | 3.52 / 7 / <!-- AI --> |
...
### 額外信號
- 🔴 HOME TTO3 penalty：OPS Δ +0.246（TTO1 0.659 → TTO3 0.905），第三輪明顯衰退
- 🟠 AWAY single-pitch dependent：主球種使用率 47.4%（≥45.0%）
- 🟠 AWAY TTO3 penalty：OPS Δ +-0.104（TTO1 0.936 → TTO3 0.832），第三輪明顯衰退；K% 從 21.0% 掉到 0.0%（Δ -21.0pp）
- 🟠 HOME chain breaks at #2-3：OPS 落差 0.267
- 🟠 AWAY chain breaks at #6-7：OPS 落差 0.216
...
- 天氣：Sunny, 76°F, wind 21 mph, In From LF
