...
- **Tier 驗證**：腳本 tier_v2 = 🔴 Elite Ace（xFIP p95, K-BB% p95），gap vs ERA-only = +31.9
...
### Brady Singer (AWAY, RHP, 29 ⚡ 巔峰期)
- **Tier 驗證**：腳本 tier_v2 = 🟡 Solid Starter（xFIP p49, K-BB% p40），gap vs ERA-only = +36.2
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 4.02 / 2 / <!-- AI --> | 4.89 / 5 / <!-- AI --> |
...
### 額外信號
- 🟠 AWAY single-pitch dependent：主球種使用率 47.7%（≥45.0%）
- 🔴 HOME chain breaks at #7-8：OPS 落差 0.434
- 🔴 AWAY chain breaks at #3-4：OPS 落差 0.384
...
- 天氣：Partly Cloudy, 76°F, wind 7 mph, Out To RF
