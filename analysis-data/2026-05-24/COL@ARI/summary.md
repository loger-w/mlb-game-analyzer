...
- **Tier 驗證**：腳本 tier_v2 = 🟡 Solid Starter（xFIP p40, K-BB% p70），gap vs ERA-only = +31.9
...
### Jose Quintana (AWAY, LHP, 37 📉📉📉 快速退化)
- **Tier 驗證**：腳本 tier_v2 = ⚪ Below Average（xFIP p5, K-BB% p5），gap vs ERA-only = -38.5
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 4.17 / 6 / <!-- AI --> | 4.27 / 7 / <!-- AI --> |
...
### 額外信號
- 🟠 HOME reverse platoon Δ +0.169（vs RHB OPS 0.823 > vs LHB OPS 0.654）— RHP 對非預期手別反而吃虧
- 🟠 HOME single-pitch dependent：主球種使用率 57.8%（≥45.0%）
- 🟠 HOME TTO3 penalty：OPS Δ +-0.021（TTO1 0.809 → TTO3 0.788），第三輪明顯衰退；K% 從 29.3% 掉到 16.7%（Δ -12.6pp）
- 🔴 AWAY reverse platoon Δ +0.239（vs LHB OPS 0.981 > vs RHB OPS 0.742）— LHP 對非預期手別反而吃虧
- 🟠 HOME chain breaks at #2-3：OPS 落差 0.239
- 🟠 AWAY chain breaks at #4-5：OPS 落差 0.252
