...
- **Tier 驗證**：腳本 tier_v2 = 🟡 Solid Starter（xFIP p64, K-BB% p51），gap vs ERA-only = +47.3
...
- **Tier 驗證**：腳本 tier_v2 = 🔴 Elite Ace（xFIP p95, K-BB% p95），gap vs ERA-only = +23.5
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 3.47 / 5 / <!-- AI --> | 2.9 / 12 / <!-- AI --> |
...
### 額外信號
- 🔴 HOME TTO3 penalty：OPS Δ +0.372（TTO1 0.601 → TTO3 0.973），第三輪明顯衰退；K% 從 26.7% 掉到 6.2%（Δ -20.5pp）（career fallback）
- 🟠 AWAY TTO3 penalty：OPS Δ +0.031（TTO1 0.633 → TTO3 0.664），第三輪明顯衰退；K% 從 27.8% 掉到 24.6%（Δ -3.2pp）
- 🟠 HOME chain breaks at #5-6：OPS 落差 0.189
- 🟠 AWAY chain breaks at #1-2：OPS 落差 0.273
