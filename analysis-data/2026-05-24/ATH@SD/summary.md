...
- **Tier 驗證**：腳本 tier_v2 = 🔴 Elite Ace（xFIP p95, K-BB% p78），gap vs ERA-only = -4.2
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 3.2 / 6 / <!-- AI --> | 4.41 / 2 / <!-- AI --> |
...
- ⚠️ HOME 打線 Flag 3 (last7 BABIP=0.206):
  - <!-- AI 補：可能回歸或可能持續？是否影響本場判斷？不自動 ±run value -->

### 額外信號
- 🟠 HOME reverse platoon Δ +0.132（vs RHB OPS 0.649 > vs LHB OPS 0.517）— RHP 對非預期手別反而吃虧
- 🔴 HOME TTO3 penalty：OPS Δ +0.539（TTO1 0.372 → TTO3 0.911），第三輪明顯衰退；K% 從 31.9% 掉到 14.0%（Δ -17.9pp）
- 🔴 AWAY reverse platoon Δ +0.344（vs RHB OPS 0.706 > vs LHB OPS 0.362）— RHP 對非預期手別反而吃虧
- 🔴 AWAY TTO3 penalty：OPS Δ +0.290（TTO1 0.716 → TTO3 1.006），第三輪明顯衰退（career fallback）
- 🟠 HOME chain breaks at #7-8：OPS 落差 0.208
- 🔴 AWAY chain breaks at #2-3：OPS 落差 0.303
