...
- **Tier 驗證**：腳本 tier_v2 = 🔴 Elite Ace（xFIP p95, K-BB% p95），gap vs ERA-only = +20.4
...
### Peter Lambert (AWAY, RHP, 29 ⚡ 巔峰期)
- **Tier 驗證**：腳本 tier_v2 = 🟠 Strong Ace（xFIP p85, K-BB% p76），gap vs ERA-only = +15.8
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 3.57 / 7 / <!-- AI --> | 5.62 / 8 / <!-- AI --> |
...
### 額外信號
- 🔴 HOME reverse platoon Δ +0.242（vs LHB OPS 0.789 > vs RHB OPS 0.547）— LHP 對非預期手別反而吃虧
- 🔴 HOME TTO3 penalty：OPS Δ +0.374（TTO1 0.597 → TTO3 0.971），第三輪明顯衰退；K% 從 29.0% 掉到 10.3%（Δ -18.7pp）
- 🔴 AWAY reverse platoon Δ +0.436（vs RHB OPS 0.833 > vs LHB OPS 0.397）— RHP 對非預期手別反而吃虧
- 🔴 HOME chain breaks at #8-9：OPS 落差 0.765
- 🟠 AWAY chain breaks at #8-9：OPS 落差 0.192
...
- 天氣：Partly Cloudy, 62°F, wind 7 mph, In From RF
