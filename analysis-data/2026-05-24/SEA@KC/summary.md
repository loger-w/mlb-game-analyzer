...
- **Tier 驗證**：腳本 tier_v2 = 🟡 Solid Starter（xFIP p67, K-BB% p63），gap vs ERA-only = +1.1
...
- **Tier 驗證**：腳本 tier_v2 = 🔴 Elite Ace（xFIP p81, K-BB% p89），gap vs ERA-only = +19.7
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 4.53 / 6 / <!-- AI --> | 3.01 / 3 / <!-- AI --> |
...
### 額外信號
- ℹ️ HOME balanced 4+ pitches：最高球種僅 21.2%（<25.0%）
- 🟠 HOME TTO3 penalty：OPS Δ +0.054（TTO1 0.747 → TTO3 0.801），第三輪明顯衰退；K% 從 22.5% 掉到 16.4%（Δ -6.1pp）
- 🟠 AWAY single-pitch dependent：主球種使用率 48.8%（≥45.0%）
- 🟠 HOME chain breaks at #2-3：OPS 落差 0.232
- 🟠 AWAY chain breaks at #5-6：OPS 落差 0.231
...
- 天氣：Sunny, 79°F, wind 5 mph, In From RF
