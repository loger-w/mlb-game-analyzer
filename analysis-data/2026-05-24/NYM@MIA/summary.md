...
### Tyler Phillips (HOME, RHP, 28 ⚡ 巔峰期)
- **Tier 驗證**：腳本 tier_v2 = 🟡 Solid Starter（xFIP p47, K-BB% p40），gap vs ERA-only = -39.7
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 3.43 / 3 / <!-- AI --> | 3.33 / 7 / <!-- AI --> |
...
- ⚠️ HOME 投手 Flag 8 (era_xera_delta=-2.24):
  - <!-- AI 補：是運氣還結構性？是否影響本場判斷？不自動下修預測 -->
- ⚠️ AWAY 打線 Flag 3 (last7 BABIP=0.256):
  - <!-- AI 補：可能回歸或可能持續？是否影響本場判斷？不自動 ±run value -->

### 額外信號
- 🔴 AWAY reverse platoon Δ +0.469（vs RHB OPS 0.993 > vs LHB OPS 0.524）— RHP 對非預期手別反而吃虧
- 🟠 AWAY single-pitch dependent：主球種使用率 55.0%（≥45.0%）
- 🟠 AWAY TTO3 penalty：OPS Δ +0.050（TTO1 0.788 → TTO3 0.838），第三輪明顯衰退；K% 從 24.0% 掉到 20.9%（Δ -3.1pp）（career fallback）
- 🟠 HOME chain breaks at #7-8：OPS 落差 0.279
- 🔴 AWAY chain breaks at #8-9：OPS 落差 0.300
