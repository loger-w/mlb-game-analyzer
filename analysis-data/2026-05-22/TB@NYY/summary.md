...
- **Tier 驗證**：腳本 tier_v2 = 🟡 Solid Starter（xFIP p54, K-BB% p56），gap vs ERA-only = -36.5
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 3.45 / 4 / <!-- AI --> | 4.33 / 6 / <!-- AI --> |
...
- ⚠️ AWAY 投手 Flag 8 (era_xera_delta=-2.37):
  - <!-- AI 補：是運氣還結構性？是否影響本場判斷？不自動下修預測 -->

### 額外信號
- 🟠 HOME single-pitch dependent：主球種使用率 61.5%（≥45.0%）
- 🟠 AWAY TTO3 penalty：OPS Δ +-0.007（TTO1 0.815 → TTO3 0.808），第三輪明顯衰退；K% 從 19.6% 掉到 7.1%（Δ -12.5pp）
- 🔴 HOME chain breaks at #4-5：OPS 落差 0.376
- 🔴 AWAY chain breaks at #4-5：OPS 落差 0.387
