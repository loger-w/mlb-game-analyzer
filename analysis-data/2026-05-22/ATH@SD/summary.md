...
- **Tier 驗證**：腳本 tier_v2 = 🟠 Strong Ace（xFIP p80, K-BB% p66），gap vs ERA-only = +41.7
...
### Jeffrey Springs (AWAY, LHP, 33 📉 初期退化)
- **Tier 驗證**：腳本 tier_v2 = 🟡 Solid Starter（xFIP p63, K-BB% p70），gap vs ERA-only = +8.1
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 3.32 / 6 / <!-- AI --> | 4.44 / 2 / <!-- AI --> |
...
### 額外信號
- ℹ️ HOME balanced 4+ pitches：最高球種僅 22.9%（<25.0%）
- 🔴 AWAY TTO3 penalty：OPS Δ +0.178（TTO1 0.653 → TTO3 0.831），第三輪明顯衰退
- 🟠 HOME chain breaks at #7-8：OPS 落差 0.262
- 🔴 AWAY chain breaks at #2-3：OPS 落差 0.322
