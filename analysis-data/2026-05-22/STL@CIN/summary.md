...
### Chris Paddack (HOME, RHP, 30 📉 初期退化)
- **Tier 驗證**：腳本 tier_v2 = 🟠 Strong Ace（xFIP p73, K-BB% p57），gap vs ERA-only = +56.0
...
- **Tier 驗證**：腳本 tier_v2 = 🟡 Solid Starter（xFIP p50, K-BB% p27），gap vs ERA-only = -8.5
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 4.98 / 5 / <!-- AI --> | 4.54 / 0 / <!-- AI --> |
...
- ⚠️ HOME 投手 Flag 8 (era_xera_delta=+2.83):
  - <!-- AI 補：是運氣還結構性？是否影響本場判斷？不自動下修預測 -->
- ⚠️ AWAY 投手 Flag 8 (era_xera_delta=-2.23):
  - <!-- AI 補：是運氣還結構性？是否影響本場判斷？不自動下修預測 -->
...
- 🔴 AWAY TTO3 penalty：OPS Δ +0.593（TTO1 0.584 → TTO3 1.177），第三輪明顯衰退；K% 從 24.4% 掉到 4.5%（Δ -19.9pp）
- 🟠 HOME chain breaks at #4-5：OPS 落差 0.270
- 🟠 AWAY chain breaks at #4-5：OPS 落差 0.261
