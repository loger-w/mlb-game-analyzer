...
### Eury Pérez (HOME, RHP, 23 📈 成長期)
- **Tier 驗證**：腳本 tier_v2 = 🟠 Strong Ace（xFIP p66, K-BB% p67），gap vs ERA-only = +45.6
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 3.47 / 3 / <!-- AI --> | 3.41 / 7 / <!-- AI --> |
...
### 額外信號
- 🔴 HOME reverse platoon Δ +0.206（vs RHB OPS 0.926 > vs LHB OPS 0.720）— RHP 對非預期手別反而吃虧
- 🟠 HOME single-pitch dependent：主球種使用率 49.3%（≥45.0%）
- 🔴 HOME TTO3 penalty：OPS Δ +0.240（TTO1 0.684 → TTO3 0.924），第三輪明顯衰退；K% 從 33.3% 掉到 23.1%（Δ -10.2pp）
- 🔴 AWAY reverse platoon Δ +0.474（vs RHB OPS 0.956 > vs LHB OPS 0.482）— RHP 對非預期手別反而吃虧
- 🟠 AWAY single-pitch dependent：主球種使用率 50.5%（≥45.0%）
- 🔴 AWAY TTO3 penalty：OPS Δ +0.173（TTO1 0.671 → TTO3 0.844），第三輪明顯衰退；K% 從 23.6% 掉到 13.0%（Δ -10.6pp）（career fallback）
- 🟠 HOME chain breaks at #1-2：OPS 落差 0.236
- 🔴 AWAY chain breaks at #6-7：OPS 落差 0.405
