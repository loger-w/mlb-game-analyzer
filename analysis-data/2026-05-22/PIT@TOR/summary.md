...
- **Tier 驗證**：腳本 tier_v2 = 🔴 Elite Ace（xFIP p95, K-BB% p95），gap vs ERA-only = +19.6
...
- **Tier 驗證**：腳本 tier_v2 = ⚪ Below Average（xFIP p5, K-BB% p5），gap vs ERA-only = -6.8
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 3.9 / 9 / <!-- AI --> | 4.13 / 2 / <!-- AI --> |
...
### 額外信號
- 🟠 HOME single-pitch dependent：主球種使用率 53.5%（≥45.0%）
- 🔴 HOME TTO3 penalty：OPS Δ +0.477（TTO1 0.436 → TTO3 0.913），第三輪明顯衰退
- 🟠 AWAY single-pitch dependent：主球種使用率 54.0%（≥45.0%）
- 🔴 AWAY TTO3 penalty：OPS Δ +0.413（TTO1 0.699 → TTO3 1.112），第三輪明顯衰退；K% 從 26.5% 掉到 14.3%（Δ -12.2pp）（career fallback）
- 🟠 AWAY chain breaks at #7-8：OPS 落差 0.300
