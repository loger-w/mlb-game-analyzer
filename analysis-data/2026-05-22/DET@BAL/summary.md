...
- **Tier 驗證**：腳本 tier_v2 = 🟢 Back-end Starter（xFIP p29, K-BB% p20），gap vs ERA-only = +8.7
...
- **Tier 驗證**：腳本 tier_v2 = 🟢 Back-end Starter（xFIP p34, K-BB% p44），gap vs ERA-only = +30.0
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 4.83 / 8 / <!-- AI --> | 3.9 / 8 / <!-- AI --> |
...
### 額外信號
- 🟠 AWAY single-pitch dependent：主球種使用率 49.1%（≥45.0%）
- 🟠 HOME chain breaks at #5-6：OPS 落差 0.242
- 🟠 AWAY chain breaks at #2-3：OPS 落差 0.215
