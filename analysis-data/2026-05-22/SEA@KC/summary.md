...
- **Tier 驗證**：腳本 tier_v2 = 🟡 Solid Starter（xFIP p49, K-BB% p60），gap vs ERA-only = +37.3
...
- **Tier 驗證**：腳本 tier_v2 = 🔴 Elite Ace（xFIP p95, K-BB% p95），gap vs ERA-only = +45.7
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 4.51 / 6 / <!-- AI --> | 3.1 / 3 / <!-- AI --> |
...
- ⚠️ AWAY 打線 Flag 3 (last7 BABIP=0.208):
  - <!-- AI 補：可能回歸或可能持續？是否影響本場判斷？不自動 ±run value -->

### 額外信號
- 🟠 HOME reverse platoon Δ +0.195（vs LHB OPS 0.952 > vs RHB OPS 0.757）— LHP 對非預期手別反而吃虧
- 🔴 AWAY TTO3 penalty：OPS Δ +0.355（TTO1 0.715 → TTO3 1.070），第三輪明顯衰退；K% 從 32.2% 掉到 23.1%（Δ -9.1pp）
- 🟠 HOME chain breaks at #1-2：OPS 落差 0.152
