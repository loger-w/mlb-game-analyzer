...
### Anthony Kay (AWAY, LHP, 31 📉 初期退化)
- **Tier 驗證**：腳本 tier_v2 = 🟢 Back-end Starter（xFIP p22, K-BB% p19），gap vs ERA-only = -10.2
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 3.08 / 4 / <!-- AI --> | 4.65 / 4 / <!-- AI --> |
...
- ⚠️ AWAY 投手 Flag 8 (era_xera_delta=-1.75):
  - <!-- AI 補：是運氣還結構性？是否影響本場判斷？不自動下修預測 -->
- ⚠️ HOME 打線 Flag 3 (last7 BABIP=0.234):
  - <!-- AI 補：可能回歸或可能持續？是否影響本場判斷？不自動 ±run value -->

### 額外信號
- 🟠 HOME TTO3 penalty：OPS Δ +0.060（TTO1 0.650 → TTO3 0.710），第三輪明顯衰退；K% 從 26.4% 掉到 19.3%（Δ -7.1pp）（career fallback）
- 🟠 AWAY TTO3 penalty：OPS Δ +-0.229（TTO1 0.870 → TTO3 0.641），第三輪明顯衰退；K% 從 18.2% 掉到 12.1%（Δ -6.1pp）
- 🟠 HOME chain breaks at #7-8：OPS 落差 0.205
- 🔴 AWAY chain breaks at #7-8：OPS 落差 0.338
