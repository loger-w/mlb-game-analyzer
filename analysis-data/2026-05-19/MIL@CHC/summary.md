...
### Jacob Misiorowski (AWAY, RHP, 24 📈 成長期)
- **Tier 驗證**：腳本 tier_v2 = 🔴 Elite Ace（xFIP p95, K-BB% p95），gap vs ERA-only = +2.3
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 3.75 / 8 / <!-- AI --> | 3.28 / 5 / <!-- AI --> |
...
### 額外信號
- 🔴 HOME TTO3 penalty：OPS Δ +0.304（TTO1 0.661 → TTO3 0.965），第三輪明顯衰退；K% 從 28.0% 掉到 24.1%（Δ -3.9pp）（career fallback）
- 🟠 AWAY single-pitch dependent：主球種使用率 61.1%（≥45.0%）
- 🟠 AWAY TTO3 penalty：OPS Δ +-0.067（TTO1 0.504 → TTO3 0.437），第三輪明顯衰退；K% 從 43.3% 掉到 31.0%（Δ -12.3pp）
- 🟠 AWAY chain breaks at #4-5：OPS 落差 0.172
