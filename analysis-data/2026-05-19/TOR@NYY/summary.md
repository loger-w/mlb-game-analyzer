...
### Will Warren (HOME, RHP, 26 ⚡ 巔峰期)
- **Tier 驗證**：腳本 tier_v2 = 🔴 Elite Ace（xFIP p95, K-BB% p95），gap vs ERA-only = +25.5
...
### Dylan Cease (AWAY, RHP, 30 📉 初期退化)
- **Tier 驗證**：腳本 tier_v2 = 🔴 Elite Ace（xFIP p95, K-BB% p95），gap vs ERA-only = +2.6
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 3.57 / 4 / <!-- AI --> | 4.13 / 8 / <!-- AI --> |
...
- ⚠️ AWAY 打線 Flag 3 (last7 BABIP=0.233):
  - <!-- AI 補：可能回歸或可能持續？是否影響本場判斷？不自動 ±run value -->

### 額外信號
- 🔴 HOME TTO3 penalty：OPS Δ +0.388（TTO1 0.517 → TTO3 0.905），第三輪明顯衰退；K% 從 38.9% 掉到 16.2%（Δ -22.7pp）
- 🔴 AWAY TTO3 penalty：OPS Δ +0.406（TTO1 0.400 → TTO3 0.806），第三輪明顯衰退；K% 從 40.0% 掉到 21.4%（Δ -18.6pp）
- 🔴 HOME chain breaks at #5-6：OPS 落差 0.521
