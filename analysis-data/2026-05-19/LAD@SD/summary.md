...
### Griffin Canning (HOME, RHP, 30 📉 初期退化)
...
### Emmet Sheehan (AWAY, RHP, 26 ⚡ 巔峰期)
- **Tier 驗證**：腳本 tier_v2 = 🔴 Elite Ace（xFIP p95, K-BB% p95），gap vs ERA-only = +49.6
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 3.38 / 6 / <!-- AI --> | 3.22 / 12 / <!-- AI --> |
...
- ⚠️ HOME 投手 Flag 8 (era_xera_delta=+6.23):
  - <!-- AI 補：是運氣還結構性？是否影響本場判斷？不自動下修預測 -->

### 額外信號
- 🟠 HOME TTO3 penalty：OPS Δ +-0.022（TTO1 0.767 → TTO3 0.745），第三輪明顯衰退；K% 從 23.6% 掉到 19.6%（Δ -4.0pp）（career fallback）
- 🟠 AWAY TTO3 penalty：OPS Δ +-0.017（TTO1 0.719 → TTO3 0.702），第三輪明顯衰退；K% 從 35.8% 掉到 17.6%（Δ -18.2pp）
- 🟠 HOME chain breaks at #7-8：OPS 落差 0.242
