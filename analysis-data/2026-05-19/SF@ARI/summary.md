...
- **Tier 驗證**：腳本 tier_v2 = 🟡 Solid Starter（xFIP p48, K-BB% p78），gap vs ERA-only = +42.4
...
### Landen Roupp (AWAY, RHP, 27 ⚡ 巔峰期)
- **Tier 驗證**：腳本 tier_v2 = 🔴 Elite Ace（xFIP p95, K-BB% p90），gap vs ERA-only = +25.1
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 4.41 / 6 / <!-- AI --> | 3.43 / 7 / <!-- AI --> |
...
### 額外信號
- 🟠 HOME reverse platoon Δ +0.145（vs RHB OPS 0.789 > vs LHB OPS 0.644）— RHP 對非預期手別反而吃虧
- 🟠 HOME single-pitch dependent：主球種使用率 58.6%（≥45.0%）
- 🟠 HOME TTO3 penalty：OPS Δ +0.032（TTO1 0.720 → TTO3 0.752），第三輪明顯衰退；K% 從 32.2% 掉到 20.5%（Δ -11.7pp）
- 🟠 AWAY reverse platoon Δ +0.115（vs RHB OPS 0.631 > vs LHB OPS 0.516）— RHP 對非預期手別反而吃虧
- 🟠 AWAY TTO3 penalty：OPS Δ +-0.090（TTO1 0.626 → TTO3 0.536），第三輪明顯衰退；K% 從 30.9% 掉到 23.1%（Δ -7.8pp）
- 🔴 HOME chain breaks at #1-2：OPS 落差 0.303
- 🟠 AWAY chain breaks at #6-7：OPS 落差 0.250
