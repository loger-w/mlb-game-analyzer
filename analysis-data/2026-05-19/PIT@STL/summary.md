...
### Matthew Liberatore (HOME, LHP, 26 ⚡ 巔峰期)
- **Tier 驗證**：腳本 tier_v2 = 🟢 Back-end Starter（xFIP p32, K-BB% p34），gap vs ERA-only = -2.6
...
### Mitch Keller (AWAY, RHP, 30 📉 初期退化)
- **Tier 驗證**：腳本 tier_v2 = 🟡 Solid Starter（xFIP p53, K-BB% p56），gap vs ERA-only = -5.4
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 4.46 / 0 / <!-- AI --> | 4.1 / 2 / <!-- AI --> |
...
### 額外信號
- 🔴 HOME reverse platoon Δ +0.328（vs LHB OPS 1.077 > vs RHB OPS 0.749）— LHP 對非預期手別反而吃虧
- 🔴 HOME TTO3 penalty：OPS Δ +0.618（TTO1 0.682 → TTO3 1.300），第三輪明顯衰退；K% 從 24.4% 掉到 7.1%（Δ -17.3pp）
- 🔴 AWAY TTO3 penalty：OPS Δ +0.379（TTO1 0.433 → TTO3 0.812），第三輪明顯衰退；K% 從 20.0% 掉到 12.5%（Δ -7.5pp）
- 🟠 HOME chain breaks at #4-5：OPS 落差 0.271
- 🔴 AWAY chain breaks at #3-4：OPS 落差 0.313
