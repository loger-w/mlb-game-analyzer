...
- **Tier 驗證**：腳本 tier_v2 = 🔴 Elite Ace（xFIP p95, K-BB% p84），gap vs ERA-only = +37.3
...
- **Tier 驗證**：腳本 tier_v2 = ⚪ Below Average（xFIP p5, K-BB% p5），gap vs ERA-only = +5.5
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 5.43 / 4 / <!-- AI --> | 4.68 / 2 / <!-- AI --> |
...
### 額外信號
- 🔴 HOME TTO3 penalty：OPS Δ +0.559（TTO1 0.437 → TTO3 0.996），第三輪明顯衰退；K% 從 26.7% 掉到 15.7%（Δ -11.0pp）
- 🔴 AWAY TTO3 penalty：OPS Δ +1.017（TTO1 0.652 → TTO3 1.669），第三輪明顯衰退；K% 從 22.7% 掉到 10.3%（Δ -12.4pp）
- 🟠 HOME chain breaks at #2-3：OPS 落差 0.175
- 🔴 AWAY chain breaks at #2-3：OPS 落差 0.347
