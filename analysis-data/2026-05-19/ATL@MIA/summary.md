...
### Braxton Garrett (HOME, LHP, 28 ⚡ 巔峰期)
...
- **Tier 驗證**：腳本 tier_v2 = 🟠 Strong Ace（xFIP p77, K-BB% p62），gap vs ERA-only = -22.1
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 3.25 / 3 / <!-- AI --> | 3.29 / 6 / <!-- AI --> |
...
- ⚠️ HOME 投手 Flag 8 (era_xera_delta=+25.84):
  - <!-- AI 補：是運氣還結構性？是否影響本場判斷？不自動下修預測 -->
- ⚠️ AWAY 投手 Flag 8 (era_xera_delta=-1.79):
  - <!-- AI 補：是運氣還結構性？是否影響本場判斷？不自動下修預測 -->
- ⚠️ AWAY 打線 Flag 3 (last7 BABIP=0.245):
  - <!-- AI 補：可能回歸或可能持續？是否影響本場判斷？不自動 ±run value -->

### 額外信號
- 🟠 HOME TTO3 penalty：OPS Δ +-0.062（TTO1 0.765 → TTO3 0.703），第三輪明顯衰退；K% 從 25.5% 掉到 18.6%（Δ -6.9pp）（career fallback）
- 🟠 AWAY TTO3 penalty：OPS Δ +0.150（TTO1 0.664 → TTO3 0.814），第三輪明顯衰退；K% 從 20.7% 掉到 15.2%（Δ -5.5pp）（career fallback）
- 🔴 HOME chain breaks at #2-3：OPS 落差 0.344
...
- 天氣：Cloudy, 84°F, wind 0 mph, None
...
| HOME | 4.2 | <!-- AI 補 --> | <!-- AI 補 --> |
| AWAY | 10.9 | <!-- AI 補 --> | <!-- AI 補 --> |
| Total | 15.100000000000001 | <!-- AI 補 --> | <!-- AI 補 --> |
