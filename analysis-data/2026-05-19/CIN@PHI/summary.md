...
- **Tier 驗證**：腳本 tier_v2 = 🔴 Elite Ace（xFIP p95, K-BB% p95），gap vs ERA-only = +61.6
...
### Chase Burns (AWAY, RHP, 23 📈 成長期)
- **Tier 驗證**：腳本 tier_v2 = 🔴 Elite Ace（xFIP p95, K-BB% p87），gap vs ERA-only = -1.5
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 3.84 / 3 / <!-- AI --> | 5.06 / 5 / <!-- AI --> |
...
- ⚠️ HOME 投手 Flag 8 (era_xera_delta=+1.90):
  - <!-- AI 補：是運氣還結構性？是否影響本場判斷？不自動下修預測 -->
- ⚠️ HOME 打線 Flag 3 (last7 BABIP=0.241):
  - <!-- AI 補：可能回歸或可能持續？是否影響本場判斷？不自動 ±run value -->

### 額外信號
- 🔴 HOME TTO3 penalty：OPS Δ +0.598（TTO1 0.425 → TTO3 1.023），第三輪明顯衰退；K% 從 31.5% 掉到 21.8%（Δ -9.7pp）
- 🟠 AWAY single-pitch dependent：主球種使用率 56.5%（≥45.0%）
- 🔴 AWAY TTO3 penalty：OPS Δ +0.674（TTO1 0.420 → TTO3 1.094），第三輪明顯衰退；K% 從 34.8% 掉到 13.6%（Δ -21.2pp）
- 🔴 HOME chain breaks at #1-2：OPS 落差 0.351
- 🟠 AWAY platoon advantage：top 5 中 4 人對 LHP OPS 較 season +0.050 以上
- 🟠 AWAY chain breaks at #4-5：OPS 落差 0.277
