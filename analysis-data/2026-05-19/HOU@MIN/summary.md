...
### Zebby Matthews (HOME, RHP, 25 ⚡ 巔峰期)
...
### Lance McCullers Jr. (AWAY, RHP, 32 📉 初期退化)
- **Tier 驗證**：腳本 tier_v2 = 🟠 Strong Ace（xFIP p76, K-BB% p63），gap vs ERA-only = +57.4
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 5.17 / 7 / <!-- AI --> | 5.82 / 8 / <!-- AI --> |
...
- ⚠️ HOME 投手 Flag 8 (era_xera_delta=-1.66):
  - <!-- AI 補：是運氣還結構性？是否影響本場判斷？不自動下修預測 -->
- ⚠️ AWAY 投手 Flag 8 (era_xera_delta=+2.48):
  - <!-- AI 補：是運氣還結構性？是否影響本場判斷？不自動下修預測 -->
- ⚠️ AWAY 打線 Flag 3 (last7 BABIP=0.253):
  - <!-- AI 補：可能回歸或可能持續？是否影響本場判斷？不自動 ±run value -->

### 額外信號
- 🟠 HOME single-pitch dependent：主球種使用率 48.2%（≥45.0%）
- 🟠 HOME TTO3 penalty：OPS Δ +-0.126（TTO1 0.799 → TTO3 0.673），第三輪明顯衰退；K% 從 24.6% 掉到 16.0%（Δ -8.6pp）（career fallback）
- 🔴 AWAY TTO3 penalty：OPS Δ +0.231（TTO1 0.746 → TTO3 0.977），第三輪明顯衰退；K% 從 24.7% 掉到 20.0%（Δ -4.7pp）
- 🟠 HOME chain breaks at #7-8：OPS 落差 0.264
- 🟠 AWAY chain breaks at #2-3：OPS 落差 0.246
