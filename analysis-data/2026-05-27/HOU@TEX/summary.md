...
- **Tier 驗證**：腳本 tier_v2 = 🔴 Elite Ace（xFIP p95, K-BB% p95），gap vs ERA-only = +27.8
...
- **Tier 驗證**：腳本 tier_v2 = 🟡 Solid Starter（xFIP p61, K-BB% p57），gap vs ERA-only = +48.5
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 3.14 / 5 / <!-- AI --> | 5.47 / 8 / <!-- AI --> |
...
- ⚠️ AWAY 投手 Flag 8 (era_xera_delta=+1.56):
  - <!-- AI 補：是運氣還結構性？是否影響本場判斷？不自動下修預測 -->
- ⚠️ AWAY 打線 Flag 3 (last7 BABIP=0.224):
  - <!-- AI 補：可能回歸或可能持續？是否影響本場判斷？不自動 ±run value -->

### 額外信號
- 🟠 HOME reverse platoon Δ +0.163（vs RHB OPS 0.774 > vs LHB OPS 0.611）— RHP 對非預期手別反而吃虧
- 🟠 HOME chain breaks at #3-4：OPS 落差 0.176
- 🟠 AWAY chain breaks at #1-2：OPS 落差 0.183
