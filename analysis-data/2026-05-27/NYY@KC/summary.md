...
- **Tier 驗證**：腳本 tier_v2 = 🟠 Strong Ace（xFIP p68, K-BB% p70），gap vs ERA-only = +33.8
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 4.89 / 6 / <!-- AI --> | 3.44 / 3 / <!-- AI --> |
...
- ⚠️ AWAY 投手 Flag 8 (era_xera_delta=-2.53):
  - <!-- AI 補：是運氣還結構性？是否影響本場判斷？不自動下修預測 -->
- ⚠️ HOME 打線 Flag 3 (last7 BABIP=0.255):
  - <!-- AI 補：可能回歸或可能持續？是否影響本場判斷？不自動 ±run value -->

### 額外信號
- 🟠 HOME reverse platoon Δ +0.118（vs LHB OPS 0.859 > vs RHB OPS 0.741）— LHP 對非預期手別反而吃虧
- 🟠 HOME TTO3 penalty：OPS Δ +0.017（TTO1 0.588 → TTO3 0.605），第三輪明顯衰退；K% 從 22.2% 掉到 17.0%（Δ -5.2pp）
- 🟠 AWAY single-pitch dependent：主球種使用率 54.1%（≥45.0%）
- 🟠 HOME chain breaks at #1-2：OPS 落差 0.161
- 🟠 AWAY chain breaks at #4-5：OPS 落差 0.289
