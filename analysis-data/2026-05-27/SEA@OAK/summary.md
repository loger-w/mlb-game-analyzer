...
- **Tier 驗證**：腳本 tier_v2 = 🟡 Solid Starter（xFIP p56, K-BB% p63），gap vs ERA-only = +6.3
...
- **Tier 驗證**：腳本 tier_v2 = 🔴 Elite Ace（xFIP p95, K-BB% p95），gap vs ERA-only = +36.2
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 4.23 / 3 / <!-- AI --> | 3.17 / 2 / <!-- AI --> |
...
- ⚠️ AWAY 打線 Flag 3 (last7 BABIP=0.247):
  - <!-- AI 補：可能回歸或可能持續？是否影響本場判斷？不自動 ±run value -->

### 額外信號
- 🔴 HOME TTO3 penalty：OPS Δ +0.162（TTO1 0.671 → TTO3 0.833），第三輪明顯衰退；K% 從 20.0% 掉到 16.1%（Δ -3.9pp）
- 🔴 AWAY TTO3 penalty：OPS Δ +0.394（TTO1 0.684 → TTO3 1.078），第三輪明顯衰退；K% 從 34.3% 掉到 21.8%（Δ -12.5pp）
- 🟠 HOME chain breaks at #3-4：OPS 落差 0.238
- 🟠 AWAY chain breaks at #4-5：OPS 落差 0.281
...
- 天氣：Partly Cloudy, 68°F, wind 5 mph, Out To LF
...
| HOME | 4.8 | <!-- AI 補 --> | <!-- AI 補 --> |
| AWAY | 5.6 | <!-- AI 補 --> | <!-- AI 補 --> |
| Total | 10.399999999999999 | <!-- AI 補 --> | <!-- AI 補 --> |
