...
### Bubba Chandler (HOME, RHP, 23 📈 成長期)
- **Tier 驗證**：腳本 tier_v2 = 🟢 Back-end Starter（xFIP p14, K-BB% p23），gap vs ERA-only = -8.7
...
- **Tier 驗證**：腳本 tier_v2 = 🟡 Solid Starter（xFIP p58, K-BB% p64），gap vs ERA-only = +32.6
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 4.0 / 2 / <!-- AI --> | 3.64 / 8 / <!-- AI --> |
...
- ⚠️ AWAY 打線 Flag 3 (last7 BABIP=0.209):
  - <!-- AI 補：可能回歸或可能持續？是否影響本場判斷？不自動 ±run value -->

### 額外信號
- 🟠 HOME single-pitch dependent：主球種使用率 52.8%（≥45.0%）
- 🔴 HOME TTO3 penalty：OPS Δ +0.555（TTO1 0.582 → TTO3 1.137），第三輪明顯衰退；K% 從 31.1% 掉到 17.6%（Δ -13.5pp）
- 🟠 AWAY TTO3 penalty：OPS Δ +-0.043（TTO1 0.826 → TTO3 0.783），第三輪明顯衰退；K% 從 23.2% 掉到 16.4%（Δ -6.8pp）
- 🔴 HOME chain breaks at #7-8：OPS 落差 0.342
...
| HOME | 7.2 | <!-- AI 補 --> | <!-- AI 補 --> |
| AWAY | 5.9 | <!-- AI 補 --> | <!-- AI 補 --> |
| Total | 13.100000000000001 | <!-- AI 補 --> | <!-- AI 補 --> |
