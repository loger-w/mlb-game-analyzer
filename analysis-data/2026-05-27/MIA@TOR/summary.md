...
### Kevin Gausman (HOME, RHP, 35 📉📉 明顯退化)
- **Tier 驗證**：腳本 tier_v2 = 🔴 Elite Ace（xFIP p95, K-BB% p95），gap vs ERA-only = +15.2
...
- **Tier 驗證**：腳本 tier_v2 = 🟠 Strong Ace（xFIP p72, K-BB% p73），gap vs ERA-only = +41.5
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 3.8 / 10 / <!-- AI --> | 3.27 / 3 / <!-- AI --> |
...
- ⚠️ AWAY 打線 Flag 3 (last7 BABIP=0.218):
  - <!-- AI 補：可能回歸或可能持續？是否影響本場判斷？不自動 ±run value -->

### 額外信號
- 🟠 HOME single-pitch dependent：主球種使用率 52.5%（≥45.0%）
- 🔴 HOME TTO3 penalty：OPS Δ +0.378（TTO1 0.468 → TTO3 0.846），第三輪明顯衰退
- 🟠 AWAY reverse platoon Δ +0.100（vs RHB OPS 0.826 > vs LHB OPS 0.726）— RHP 對非預期手別反而吃虧
- 🟠 AWAY single-pitch dependent：主球種使用率 48.5%（≥45.0%）
- 🔴 AWAY TTO3 penalty：OPS Δ +0.211（TTO1 0.674 → TTO3 0.885），第三輪明顯衰退；K% 從 32.4% 掉到 22.2%（Δ -10.2pp）
- 🟠 HOME chain breaks at #8-9：OPS 落差 0.230
- 🟠 AWAY chain breaks at #7-8：OPS 落差 0.222
...
- 天氣：Sunny, 66°F, wind 8 mph, Out To CF
