...
- **Tier 驗證**：腳本 tier_v2 = 🟠 Strong Ace（xFIP p78, K-BB% p72），gap vs ERA-only = +8.5
...
### Bryce Elder (AWAY, RHP, 27 ⚡ 巔峰期)
- **Tier 驗證**：腳本 tier_v2 = 🟠 Strong Ace（xFIP p81, K-BB% p75），gap vs ERA-only = -11.6
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 3.09 / 5 / <!-- AI --> | 3.15 / 6 / <!-- AI --> |
...
### 額外信號
- 🔴 AWAY reverse platoon Δ +0.233（vs RHB OPS 0.696 > vs LHB OPS 0.463）— RHP 對非預期手別反而吃虧
- 🟠 AWAY TTO3 penalty：OPS Δ +0.084（TTO1 0.437 → TTO3 0.521），第三輪明顯衰退；K% 從 27.3% 掉到 18.8%（Δ -8.5pp）
- 🔴 HOME chain breaks at #4-5：OPS 落差 0.302
