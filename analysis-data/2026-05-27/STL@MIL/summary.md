...
### Chad Patrick (HOME, RHP, 27 ⚡ 巔峰期)
- **Tier 驗證**：腳本 tier_v2 = 🟢 Back-end Starter（xFIP p22, K-BB% p30），gap vs ERA-only = -45.1
...
### Dustin May (AWAY, RHP, 28 ⚡ 巔峰期)
- **Tier 驗證**：腳本 tier_v2 = 🟠 Strong Ace（xFIP p69, K-BB% p55），gap vs ERA-only = +35.0
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 3.34 / 6 / <!-- AI --> | 4.37 / 0 / <!-- AI --> |
...
### 額外信號
- 🟠 HOME TTO3 penalty：OPS Δ +0.137（TTO1 0.734 → TTO3 0.871），第三輪明顯衰退；K% 從 24.5% 掉到 19.2%（Δ -5.3pp）（career fallback）
- 🟠 AWAY TTO3 penalty：OPS Δ +-0.056（TTO1 0.785 → TTO3 0.729），第三輪明顯衰退；K% 從 21.1% 掉到 15.1%（Δ -6.0pp）
- 🟠 HOME chain breaks at #6-7：OPS 落差 0.191
...
- 天氣：Partly Cloudy, 69°F, wind 7 mph, L To R
