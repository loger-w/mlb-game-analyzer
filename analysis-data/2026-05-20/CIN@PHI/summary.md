...
- **Tier 驗證**：腳本 tier_v2 = 🟠 Strong Ace（xFIP p89, K-BB% p73），gap vs ERA-only = +64.9
...
- **Tier 驗證**：腳本 tier_v2 = 🟢 Back-end Starter（xFIP p22, K-BB% p20），gap vs ERA-only = -16.7
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 3.88 / 3 / <!-- AI --> | 4.98 / 5 / <!-- AI --> |
...
- ⚠️ HOME 打線 Flag 3 (last7 BABIP=0.221):
  - <!-- AI 補：可能回歸或可能持續？是否影響本場判斷？不自動 ±run value -->

### 額外信號
- 🟠 HOME TTO3 penalty：OPS Δ +0.148（TTO1 0.862 → TTO3 1.010），第三輪明顯衰退
- 🟠 AWAY reverse platoon Δ +0.084（vs LHB OPS 0.798 > vs RHB OPS 0.714）— LHP 對非預期手別反而吃虧
- 🟠 AWAY single-pitch dependent：主球種使用率 46.8%（≥45.0%）
- 🔴 HOME chain breaks at #8-9：OPS 落差 0.431
...
- 天氣：Sunny, 90°F, wind 11 mph, Out To RF
