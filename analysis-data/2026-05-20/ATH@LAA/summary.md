...
- **Tier 驗證**：腳本 tier_v2 = 🟢 Back-end Starter（xFIP p33, K-BB% p14），gap vs ERA-only = -5.0
...
- **Tier 驗證**：腳本 tier_v2 = 🟡 Solid Starter（xFIP p37, K-BB% p51），gap vs ERA-only = -32.4
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 5.63 / 4 / <!-- AI --> | 4.64 / 2 / <!-- AI --> |
...
- ⚠️ AWAY 投手 Flag 8 (era_xera_delta=-1.69):
  - <!-- AI 補：是運氣還結構性？是否影響本場判斷？不自動下修預測 -->

### 額外信號
- 🟠 HOME TTO3 penalty：OPS Δ +0.067（TTO1 0.515 → TTO3 0.582），第三輪明顯衰退；K% 從 21.1% 掉到 12.5%（Δ -8.6pp）
- 🔴 AWAY TTO3 penalty：OPS Δ +0.197（TTO1 0.682 → TTO3 0.879），第三輪明顯衰退；K% 從 25.6% 掉到 9.8%（Δ -15.8pp）
- 🟠 HOME chain breaks at #2-3：OPS 落差 0.212
- 🔴 AWAY chain breaks at #2-3：OPS 落差 0.339
