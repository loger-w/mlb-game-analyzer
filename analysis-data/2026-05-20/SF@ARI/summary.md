...
- **Tier 驗證**：腳本 tier_v2 = ⚪ Below Average（xFIP p5, K-BB% p10），gap vs ERA-only = +4.2
...
- **Tier 驗證**：腳本 tier_v2 = 🟠 Strong Ace（xFIP p87, K-BB% p68），gap vs ERA-only = +56.6
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 4.36 / 6 / <!-- AI --> | 3.54 / 7 / <!-- AI --> |
...
- ⚠️ HOME 投手 Flag 8 (era_xera_delta=-2.01):
  - <!-- AI 補：是運氣還結構性？是否影響本場判斷？不自動下修預測 -->

### 額外信號
- 🔴 AWAY reverse platoon Δ +0.245（vs RHB OPS 0.961 > vs LHB OPS 0.716）— RHP 對非預期手別反而吃虧
- 🟠 AWAY single-pitch dependent：主球種使用率 47.1%（≥45.0%）
- 🔴 AWAY TTO3 penalty：OPS Δ +0.167（TTO1 0.839 → TTO3 1.006），第三輪明顯衰退；K% 從 24.4% 掉到 13.3%（Δ -11.1pp）
- 🔴 HOME chain breaks at #1-2：OPS 落差 0.303
- 🟠 AWAY chain breaks at #6-7：OPS 落差 0.267
