...
### Edward Cabrera (HOME, RHP, 28 ⚡ 巔峰期)
- **Tier 驗證**：腳本 tier_v2 = 🟠 Strong Ace（xFIP p83, K-BB% p66），gap vs ERA-only = +22.4
...
### Kyle Harrison (AWAY, LHP, 24 📈 成長期)
- **Tier 驗證**：腳本 tier_v2 = 🔴 Elite Ace（xFIP p95, K-BB% p95），gap vs ERA-only = +2.0
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 3.76 / 7 / <!-- AI --> | 3.33 / 5 / <!-- AI --> |
...
### 額外信號
- 🟠 HOME TTO3 penalty：OPS Δ +0.148（TTO1 0.660 → TTO3 0.808），第三輪明顯衰退
- 🟠 AWAY reverse platoon Δ +0.080（vs LHB OPS 0.697 > vs RHB OPS 0.617）— LHP 對非預期手別反而吃虧
- 🟠 AWAY single-pitch dependent：主球種使用率 58.7%（≥45.0%）
- 🟠 AWAY TTO3 penalty：OPS Δ +0.046（TTO1 0.779 → TTO3 0.825），第三輪明顯衰退；K% 從 26.6% 掉到 17.9%（Δ -8.7pp）（career fallback）
- 🟠 AWAY chain breaks at #5-6：OPS 落差 0.180
