...
### Simeon Woods Richardson (HOME, RHP, 25 ⚡ 巔峰期)
- **Tier 驗證**：腳本 tier_v2 = ⚪ Below Average（xFIP p5, K-BB% p5），gap vs ERA-only = +10.4
...
### Max Meyer (AWAY, RHP, 27 ⚡ 巔峰期)
- **Tier 驗證**：腳本 tier_v2 = 🔴 Elite Ace（xFIP p95, K-BB% p85），gap vs ERA-only = +8.7
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 5.54 / 7 / <!-- AI --> | 3.32 / 3 / <!-- AI --> |
...
- ⚠️ AWAY 打線 Flag 3 (last7 BABIP=0.223):
  - <!-- AI 補：可能回歸或可能持續？是否影響本場判斷？不自動 ±run value -->

### 額外信號
- 🟠 HOME reverse platoon Δ +0.148（vs RHB OPS 1.010 > vs LHB OPS 0.862）— RHP 對非預期手別反而吃虧
- 🔴 HOME TTO3 penalty：OPS Δ +0.215（TTO1 0.738 → TTO3 0.953），第三輪明顯衰退
- 🔴 AWAY TTO3 penalty：OPS Δ +0.240（TTO1 0.555 → TTO3 0.795），第三輪明顯衰退；K% 從 26.4% 掉到 15.6%（Δ -10.8pp）
- 🟠 HOME chain breaks at #8-9：OPS 落差 0.250
- 🟠 AWAY chain breaks at #4-5：OPS 落差 0.193
