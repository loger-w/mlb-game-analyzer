...
- **Tier 驗證**：腳本 tier_v2 = 🟡 Solid Starter（xFIP p58, K-BB% p60），gap vs ERA-only = +53.3
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 3.28 / 6 / <!-- AI --> | 3.92 / 3 / <!-- AI --> |
...
- ⚠️ AWAY 投手 Flag 8 (era_xera_delta=+1.89):
  - <!-- AI 補：是運氣還結構性？是否影響本場判斷？不自動下修預測 -->

### 額外信號
- 🔴 HOME reverse platoon Δ +0.353（vs RHB OPS 0.910 > vs LHB OPS 0.557）— RHP 對非預期手別反而吃虧
- ℹ️ HOME balanced 4+ pitches：最高球種僅 23.4%（<25.0%）
- 🔴 HOME TTO3 penalty：OPS Δ +0.186（TTO1 0.600 → TTO3 0.786），第三輪明顯衰退；K% 從 27.4% 掉到 22.3%（Δ -5.1pp）（career fallback）
- 🔴 AWAY reverse platoon Δ +0.355（vs RHB OPS 1.097 > vs LHB OPS 0.742）— RHP 對非預期手別反而吃虧
- 🔴 HOME chain breaks at #1-2：OPS 落差 0.313
- 🔴 AWAY chain breaks at #1-2：OPS 落差 0.339
