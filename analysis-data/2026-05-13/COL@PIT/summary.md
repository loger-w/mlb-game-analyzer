...
### Mitch Keller (HOME, RHP, 30 📉 初期退化)
- **Tier 驗證**：腳本 tier_v2 = 🟡 Solid Starter（xFIP p56, K-BB% p57），gap vs ERA-only = -18.4
...
- **Tier 驗證**：腳本 tier_v2 = ⚪ Below Average（xFIP p5, K-BB% p5），gap vs ERA-only = -42.1
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 4.17 / 2 / <!-- AI --> | 4.41 / 4 / <!-- AI --> |
...
### 額外信號
- 🔴 HOME TTO3 penalty：OPS Δ +0.216（TTO1 0.484 → TTO3 0.700），第三輪明顯衰退；K% 從 19.8% 掉到 14.3%（Δ -5.5pp）
- 🟠 HOME chain breaks at #8-9：OPS 落差 0.278
- 🔴 AWAY chain breaks at #6-7：OPS 落差 0.354
