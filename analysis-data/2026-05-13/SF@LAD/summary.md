...
### Shohei Ohtani (HOME, RHP, 31 📉 初期退化)
- **Tier 驗證**：腳本 tier_v2 = 🔴 Elite Ace（xFIP p95, K-BB% p95），gap vs ERA-only = -2.4
...
### Robbie Ray (AWAY, LHP, 34 📉📉 明顯退化)
- **Tier 驗證**：腳本 tier_v2 = 🟠 Strong Ace（xFIP p67, K-BB% p81），gap vs ERA-only = -11.0
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 3.66 / 10 / <!-- AI --> | 3.55 / 8 / <!-- AI --> |
...
### 額外信號
- 🟠 HOME TTO3 penalty：OPS Δ +0.044（TTO1 0.463 → TTO3 0.507），第三輪明顯衰退；K% 從 41.5% 掉到 36.1%（Δ -5.4pp）
- 🟠 AWAY single-pitch dependent：主球種使用率 48.8%（≥45.0%）
- 🟠 AWAY TTO3 penalty：OPS Δ +0.088（TTO1 0.696 → TTO3 0.784），第三輪明顯衰退；K% 從 30.5% 掉到 17.9%（Δ -12.6pp）
- 🟠 HOME chain breaks at #5-6：OPS 落差 0.228
- 🟠 AWAY chain breaks at #8-9：OPS 落差 0.212
...
| HOME | 5.4 | <!-- AI 補 --> | <!-- AI 補 --> |
| AWAY | 2.2 | <!-- AI 補 --> | <!-- AI 補 --> |
| Total | 7.6000000000000005 | <!-- AI 補 --> | <!-- AI 補 --> |
