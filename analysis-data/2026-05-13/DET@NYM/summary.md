...
### Christian Scott (HOME, RHP, 26 ⚡ 巔峰期)
...
### Framber Valdez (AWAY, LHP, 32 📉 初期退化)
- **Tier 驗證**：腳本 tier_v2 = 🟡 Solid Starter（xFIP p77, K-BB% p51），gap vs ERA-only = +23.0
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 3.64 / 6 / <!-- AI --> | 3.95 / 10 / <!-- AI --> |
...
### 額外信號
- 🟠 HOME single-pitch dependent：主球種使用率 55.3%（≥45.0%）
- 🟠 HOME TTO3 penalty：OPS Δ +0.113（TTO1 0.757 → TTO3 0.870），第三輪明顯衰退（career fallback）
- 🟠 AWAY single-pitch dependent：主球種使用率 46.2%（≥45.0%）
- 🟠 HOME chain breaks at #7-8：OPS 落差 0.295
- 🟠 AWAY chain breaks at #2-3：OPS 落差 0.177
...
| HOME | 4.1 | <!-- AI 補 --> | <!-- AI 補 --> |
| AWAY | 3.8 | <!-- AI 補 --> | <!-- AI 補 --> |
| Total | 7.8999999999999995 | <!-- AI 補 --> | <!-- AI 補 --> |
