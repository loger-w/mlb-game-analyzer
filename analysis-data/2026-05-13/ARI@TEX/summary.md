...
- **Tier 驗證**：腳本 tier_v2 = 🟡 Solid Starter（xFIP p73, K-BB% p47），gap vs ERA-only = +35.0
...
### Ryne Nelson (AWAY, RHP, 28 ⚡ 巔峰期)
- **Tier 驗證**：腳本 tier_v2 = 🟡 Solid Starter（xFIP p32, K-BB% p66），gap vs ERA-only = +36.7
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 2.84 / 6 / <!-- AI --> | 4.24 / 6 / <!-- AI --> |
...
- ⚠️ HOME 打線 Flag 3 (last7 BABIP=0.229):
  - <!-- AI 補：可能回歸或可能持續？是否影響本場判斷？不自動 ±run value -->
- ⚠️ AWAY 打線 Flag 3 (last7 BABIP=0.177):
  - <!-- AI 補：可能回歸或可能持續？是否影響本場判斷？不自動 ±run value -->

### 額外信號
- 🟠 AWAY reverse platoon Δ +0.098（vs RHB OPS 0.797 > vs LHB OPS 0.699）— RHP 對非預期手別反而吃虧
- 🟠 AWAY single-pitch dependent：主球種使用率 61.1%（≥45.0%）
- 🟠 AWAY TTO3 penalty：OPS Δ +0.078（TTO1 0.777 → TTO3 0.855），第三輪明顯衰退；K% 從 30.9% 掉到 19.4%（Δ -11.5pp）
- 🟠 HOME chain breaks at #4-5：OPS 落差 0.249
- 🟠 AWAY chain breaks at #5-6：OPS 落差 0.235
