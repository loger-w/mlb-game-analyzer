...
- **Tier 驗證**：腳本 tier_v2 = 🟠 Strong Ace（xFIP p95, K-BB% p64），gap vs ERA-only = +44.2
...
- **Tier 驗證**：腳本 tier_v2 = 🟠 Strong Ace（xFIP p75, K-BB% p69），gap vs ERA-only = -7.3
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 4.16 / 8 / <!-- AI --> | 3.2 / 3 / <!-- AI --> |
...
### 額外信號
- 🟠 HOME reverse platoon Δ +0.161（vs RHB OPS 0.941 > vs LHB OPS 0.780）— RHP 對非預期手別反而吃虧
- ℹ️ AWAY balanced 4+ pitches：最高球種僅 22.4%（<25.0%）
- 🟠 AWAY TTO3 penalty：OPS Δ +-0.156（TTO1 0.557 → TTO3 0.401），第三輪明顯衰退；K% 從 30.5% 掉到 11.9%（Δ -18.6pp）
- 🟠 HOME chain breaks at #6-7：OPS 落差 0.217
- 🔴 AWAY chain breaks at #6-7：OPS 落差 0.335
...
- 天氣：Sunny, 73°F, wind 14 mph, Out To LF
