...
- **Tier 驗證**：腳本 tier_v2 = 🟡 Solid Starter（xFIP p70, K-BB% p58），gap vs ERA-only = +53.4
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 6.02 / 8 / <!-- AI --> | 3.23 / 5 / <!-- AI --> |
...
- ⚠️ HOME 投手 Flag 8 (era_xera_delta=+2.91):
  - <!-- AI 補：是運氣還結構性？是否影響本場判斷？不自動下修預測 -->
- ⚠️ HOME 打線 Flag 3 (last7 BABIP=0.231):
  - <!-- AI 補：可能回歸或可能持續？是否影響本場判斷？不自動 ±run value -->

### 額外信號
- 🟠 HOME TTO3 penalty：OPS Δ +0.132（TTO1 0.732 → TTO3 0.864），第三輪明顯衰退（career fallback）
- 🟠 AWAY TTO3 penalty：OPS Δ +0.045（TTO1 0.651 → TTO3 0.696），第三輪明顯衰退；K% 從 26.7% 掉到 19.4%（Δ -7.3pp）（career fallback）
- 🟠 HOME chain breaks at #7-8：OPS 落差 0.266
- 🔴 AWAY chain breaks at #7-8：OPS 落差 0.402
