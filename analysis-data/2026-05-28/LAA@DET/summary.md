...
- **Tier 驗證**：腳本 tier_v2 = 🟡 Solid Starter（xFIP p55, K-BB% p58），gap vs ERA-only = +47.3
...
### Grayson Rodriguez (AWAY, RHP, 26 ⚡ 巔峰期)
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 4.02 / 9 / <!-- AI --> | 5.24 / 3 / <!-- AI --> |
...
- ⚠️ AWAY 投手 Flag 8 (era_xera_delta=+2.36):
  - <!-- AI 補：是運氣還結構性？是否影響本場判斷？不自動下修預測 -->
- ⚠️ AWAY 打線 Flag 3 (last7 BABIP=0.249):
  - <!-- AI 補：可能回歸或可能持續？是否影響本場判斷？不自動 ±run value -->

### 額外信號
- 🟠 HOME single-pitch dependent：主球種使用率 49.4%（≥45.0%）
- 🟠 HOME TTO3 penalty：OPS Δ +-0.177（TTO1 0.881 → TTO3 0.704），第三輪明顯衰退；K% 從 30.3% 掉到 27.3%（Δ -3.0pp）
- 🟠 AWAY single-pitch dependent：主球種使用率 49.4%（≥45.0%）
- 🟠 AWAY TTO3 penalty：OPS Δ +0.109（TTO1 0.718 → TTO3 0.827），第三輪明顯衰退（career fallback）
- 🟠 HOME chain breaks at #5-6：OPS 落差 0.267
- 🔴 AWAY chain breaks at #5-6：OPS 落差 0.435
...
- 天氣：Sunny, 61°F, wind 14 mph, Out To RF
