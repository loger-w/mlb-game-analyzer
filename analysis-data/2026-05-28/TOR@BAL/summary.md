...
### Chris Bassitt (HOME, RHP, 37 📉📉📉 快速退化)
- **Tier 驗證**：腳本 tier_v2 = 🟢 Back-end Starter（xFIP p41, K-BB% p26），gap vs ERA-only = +17.3
...
- **Tier 驗證**：腳本 tier_v2 = 🟡 Solid Starter（xFIP p69, K-BB% p50），gap vs ERA-only = +0.7
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 4.56 / 7 / <!-- AI --> | 3.73 / 10 / <!-- AI --> |
...
- ⚠️ AWAY 投手 Flag 8 (era_xera_delta=-1.52):
  - <!-- AI 補：是運氣還結構性？是否影響本場判斷？不自動下修預測 -->

### 額外信號
- 🟠 AWAY TTO3 penalty：OPS Δ +0.067（TTO1 0.824 → TTO3 0.891），第三輪明顯衰退；K% 從 20.5% 掉到 14.2%（Δ -6.3pp）（career fallback）
- 🟠 HOME chain breaks at #7-8：OPS 落差 0.159
- 🟠 AWAY chain breaks at #7-8：OPS 落差 0.167
