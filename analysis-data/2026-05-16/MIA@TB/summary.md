...
### Nick Martinez (HOME, RHP, 35 📉📉 明顯退化)
- **Tier 驗證**：腳本 tier_v2 = 🟡 Solid Starter（xFIP p52, K-BB% p55），gap vs ERA-only = -37.9
...
### Sandy Alcantara (AWAY, RHP, 30 📉 初期退化)
- **Tier 驗證**：腳本 tier_v2 = 🟢 Back-end Starter（xFIP p43, K-BB% p34），gap vs ERA-only = -11.2
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 4.12 / 7 / <!-- AI --> | 3.29 / 3 / <!-- AI --> |
...
- ⚠️ HOME 投手 Flag 8 (era_xera_delta=-2.12):
  - <!-- AI 補：是運氣還結構性？是否影響本場判斷？不自動下修預測 -->
- ⚠️ AWAY 打線 Flag 3 (last7 BABIP=0.213):
  - <!-- AI 補：可能回歸或可能持續？是否影響本場判斷？不自動 ±run value -->

### 額外信號
- 🟠 HOME TTO3 penalty：OPS Δ +-0.010（TTO1 0.839 → TTO3 0.829），第三輪明顯衰退；K% 從 19.3% 掉到 7.8%（Δ -11.5pp）
- ℹ️ AWAY balanced 4+ pitches：最高球種僅 23.1%（<25.0%）
- 🔴 AWAY TTO3 penalty：OPS Δ +0.269（TTO1 0.507 → TTO3 0.776），第三輪明顯衰退；K% 從 22.6% 掉到 15.7%（Δ -6.9pp）
- 🟠 HOME chain breaks at #4-5：OPS 落差 0.191
