...
### Jacob deGrom (AWAY, RHP, 37 📉📉📉 快速退化)
- **Tier 驗證**：腳本 tier_v2 = 🔴 Elite Ace（xFIP p95, K-BB% p95），gap vs ERA-only = +2.2
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 5.81 / 8 / <!-- AI --> | 3.11 / 6 / <!-- AI --> |
...
- ⚠️ HOME 打線 Flag 3 (last7 BABIP=0.23):
  - <!-- AI 補：可能回歸或可能持續？是否影響本場判斷？不自動 ±run value -->
- ⚠️ AWAY 打線 Flag 3 (last7 BABIP=0.21):
  - <!-- AI 補：可能回歸或可能持續？是否影響本場判斷？不自動 ±run value -->

### 額外信號
- 🟠 AWAY reverse platoon Δ +0.170（vs RHB OPS 0.690 > vs LHB OPS 0.520）— RHP 對非預期手別反而吃虧
- 🟠 AWAY single-pitch dependent：主球種使用率 46.2%（≥45.0%）
- 🔴 AWAY TTO3 penalty：OPS Δ +0.209（TTO1 0.656 → TTO3 0.865），第三輪明顯衰退
- 🔴 HOME chain breaks at #8-9：OPS 落差 0.461
- 🟠 AWAY chain breaks at #4-5：OPS 落差 0.239
