...
### Tomoyuki Sugano (HOME, RHP, 36 📉📉 明顯退化)
- **Tier 驗證**：腳本 tier_v2 = 🟢 Back-end Starter（xFIP p41, K-BB% p28），gap vs ERA-only = -14.7
...
- **Tier 驗證**：腳本 tier_v2 = 🟢 Back-end Starter（xFIP p25, K-BB% p20），gap vs ERA-only = -57.9
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 4.35 / 6 / <!-- AI --> | 4.38 / 6 / <!-- AI --> |
...
- ⚠️ HOME 投手 Flag 8 (era_xera_delta=-2.70):
  - <!-- AI 補：是運氣還結構性？是否影響本場判斷？不自動下修預測 -->
- ⚠️ AWAY 投手 Flag 8 (era_xera_delta=-2.34):
  - <!-- AI 補：是運氣還結構性？是否影響本場判斷？不自動下修預測 -->
- ⚠️ AWAY 打線 Flag 3 (last7 BABIP=0.231):
  - <!-- AI 補：可能回歸或可能持續？是否影響本場判斷？不自動 ±run value -->

### 額外信號
- ℹ️ HOME balanced 4+ pitches：最高球種僅 24.9%（<25.0%）
- 🔴 HOME chain breaks at #6-7：OPS 落差 0.417
- 🟠 AWAY chain breaks at #5-6：OPS 落差 0.261
