...
- **Tier 驗證**：腳本 tier_v2 = ⚪ Below Average（xFIP p5, K-BB% p14），gap vs ERA-only = -15.7
...
### Cristopher Sánchez (AWAY, LHP, 29 ⚡ 巔峰期)
- **Tier 驗證**：腳本 tier_v2 = 🔴 Elite Ace（xFIP p95, K-BB% p95），gap vs ERA-only = +0.5
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 4.23 / 2 / <!-- AI --> | 3.85 / 3 / <!-- AI --> |
...
### 額外信號
- 🟠 HOME single-pitch dependent：主球種使用率 55.2%（≥45.0%）
- 🔴 HOME TTO3 penalty：OPS Δ +0.434（TTO1 0.678 → TTO3 1.112），第三輪明顯衰退；K% 從 26.7% 掉到 14.3%（Δ -12.4pp）（career fallback）
- 🔴 HOME chain breaks at #4-5：OPS 落差 0.318
- 🔴 AWAY chain breaks at #1-2：OPS 落差 0.394
