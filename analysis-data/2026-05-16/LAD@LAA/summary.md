...
- **Tier 驗證**：腳本 tier_v2 = 🟢 Back-end Starter（xFIP p19, K-BB% p23），gap vs ERA-only = -52.4
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 5.17 / 5 / <!-- AI --> | 3.35 / 11 / <!-- AI --> |
...
- ⚠️ HOME 投手 Flag 8 (era_xera_delta=-2.10):
  - <!-- AI 補：是運氣還結構性？是否影響本場判斷？不自動下修預測 -->
- ⚠️ AWAY 投手 Flag 8 (era_xera_delta=-1.64):
  - <!-- AI 補：是運氣還結構性？是否影響本場判斷？不自動下修預測 -->
- ⚠️ AWAY 打線 Flag 3 (last7 BABIP=0.246):
  - <!-- AI 補：可能回歸或可能持續？是否影響本場判斷？不自動 ±run value -->

### 額外信號
- 🔴 AWAY reverse platoon Δ +0.328（vs LHB OPS 0.778 > vs RHB OPS 0.450）— LHP 對非預期手別反而吃虧
- 🟠 AWAY single-pitch dependent：主球種使用率 50.6%（≥45.0%）
- 🟠 AWAY TTO3 penalty：OPS Δ +-0.337（TTO1 0.641 → TTO3 0.304），第三輪明顯衰退；K% 從 17.6% 掉到 13.6%（Δ -4.0pp）
- 🟠 HOME chain breaks at #2-3：OPS 落差 0.204
- 🟠 AWAY chain breaks at #5-6：OPS 落差 0.174
