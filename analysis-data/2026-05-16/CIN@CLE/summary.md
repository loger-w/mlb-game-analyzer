...
### Joey Cantillo (HOME, LHP, 26 ⚡ 巔峰期)
- **Tier 驗證**：腳本 tier_v2 = 🟡 Solid Starter（xFIP p61, K-BB% p57），gap vs ERA-only = -11.4
...
### Chris Paddack (AWAY, RHP, 30 📉 初期退化)
- **Tier 驗證**：腳本 tier_v2 = 🟠 Strong Ace（xFIP p70, K-BB% p60），gap vs ERA-only = +55.6
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 4.12 / 2 / <!-- AI --> | 4.76 / 5 / <!-- AI --> |
...
- ⚠️ AWAY 投手 Flag 8 (era_xera_delta=+3.31):
  - <!-- AI 補：是運氣還結構性？是否影響本場判斷？不自動下修預測 -->
- ⚠️ HOME 打線 Flag 3 (last7 BABIP=0.223):
  - <!-- AI 補：可能回歸或可能持續？是否影響本場判斷？不自動 ±run value -->

### 額外信號
- 🟠 HOME single-pitch dependent：主球種使用率 45.0%（≥45.0%）
- 🔴 HOME TTO3 penalty：OPS Δ +0.229（TTO1 0.624 → TTO3 0.853），第三輪明顯衰退；K% 從 28.2% 掉到 21.1%（Δ -7.1pp）（career fallback）
- 🟠 AWAY TTO3 penalty：OPS Δ +-0.017（TTO1 0.732 → TTO3 0.715），第三輪明顯衰退；K% 從 21.6% 掉到 17.7%（Δ -3.9pp）（career fallback）
- 🔴 HOME chain breaks at #8-9：OPS 落差 0.376
- 🟠 AWAY platoon advantage：top 5 中 4 人對 LHP OPS 較 season +0.050 以上
- 🟠 AWAY chain breaks at #4-5：OPS 落差 0.244
