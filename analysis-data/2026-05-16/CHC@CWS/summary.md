...
- **Tier 驗證**：腳本 tier_v2 = 🟡 Solid Starter（xFIP p57, K-BB% p67），gap vs ERA-only = +3.6
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 4.63 / 4 / <!-- AI --> | 3.75 / 8 / <!-- AI --> |
...
- ⚠️ HOME 投手 Flag 8 (era_xera_delta=-2.28):
  - <!-- AI 補：是運氣還結構性？是否影響本場判斷？不自動下修預測 -->
- ⚠️ AWAY 打線 Flag 3 (last7 BABIP=0.237):
  - <!-- AI 補：可能回歸或可能持續？是否影響本場判斷？不自動 ±run value -->

### 額外信號
- 🔴 HOME TTO3 penalty：OPS Δ +0.242（TTO1 0.471 → TTO3 0.713），第三輪明顯衰退；K% 從 30.6% 掉到 16.7%（Δ -13.9pp）
- 🟠 AWAY TTO3 penalty：OPS Δ +-0.180（TTO1 0.789 → TTO3 0.609），第三輪明顯衰退；K% 從 24.7% 掉到 20.5%（Δ -4.2pp）
- 🔴 HOME chain breaks at #8-9：OPS 落差 0.362
- 🟠 AWAY chain breaks at #3-4：OPS 落差 0.154
