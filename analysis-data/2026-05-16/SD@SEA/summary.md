...
### Logan Gilbert (HOME, RHP, 29 ⚡ 巔峰期)
- **Tier 驗證**：腳本 tier_v2 = 🔴 Elite Ace（xFIP p95, K-BB% p95），gap vs ERA-only = +31.0
...
### Walker Buehler (AWAY, RHP, 31 📉 初期退化)
- **Tier 驗證**：腳本 tier_v2 = 🟠 Strong Ace（xFIP p80, K-BB% p61），gap vs ERA-only = +43.4
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 3.08 / 4 / <!-- AI --> | 3.47 / 6 / <!-- AI --> |
...
### 額外信號
- 🔴 HOME TTO3 penalty：OPS Δ +0.376（TTO1 0.733 → TTO3 1.109），第三輪明顯衰退；K% 從 33.3% 掉到 25.6%（Δ -7.7pp）
- ℹ️ AWAY balanced 4+ pitches：最高球種僅 22.7%（<25.0%）
- 🔴 HOME chain breaks at #6-7：OPS 落差 0.476
- 🟠 AWAY chain breaks at #7-8：OPS 落差 0.250
