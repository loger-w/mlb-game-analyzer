...
### Taj Bradley (AWAY, RHP, 25 ⚡ 巔峰期)
- **Tier 驗證**：腳本 tier_v2 = 🔴 Elite Ace（xFIP p86, K-BB% p86），gap vs ERA-only = +7.6
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 3.13 / 6 / <!-- AI --> | 4.86 / 6 / <!-- AI --> |
...
- ⚠️ HOME 打線 Flag 3 (last7 BABIP=0.38):
  - <!-- AI 補：可能回歸或可能持續？是否影響本場判斷？不自動 ±run value -->

### 額外信號
- 🟠 AWAY single-pitch dependent：主球種使用率 47.4%（≥45.0%）
- 🟠 HOME chain breaks at #6-7：OPS 落差 0.274
- 🔴 AWAY chain breaks at #7-8：OPS 落差 0.324
...
- 天氣：Partly Cloudy, 61°F, wind 9 mph, In From RF
