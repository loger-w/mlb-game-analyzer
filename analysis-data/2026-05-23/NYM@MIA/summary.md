...
- **Tier 驗證**：腳本 tier_v2 = 🔴 Elite Ace（xFIP p95, K-BB% p89），gap vs ERA-only = +11.5
...
### Freddy Peralta (AWAY, RHP, 29 ⚡ 巔峰期)
- **Tier 驗證**：腳本 tier_v2 = 🟠 Strong Ace（xFIP p66, K-BB% p64），gap vs ERA-only = -1.6
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 3.42 / 3 / <!-- AI --> | 3.35 / 7 / <!-- AI --> |
...
### 額外信號
- 🔴 HOME TTO3 penalty：OPS Δ +0.262（TTO1 0.559 → TTO3 0.821），第三輪明顯衰退；K% 從 27.8% 掉到 17.8%（Δ -10.0pp）
- 🟠 AWAY single-pitch dependent：主球種使用率 56.2%（≥45.0%）
- 🟠 AWAY TTO3 penalty：OPS Δ +-0.101（TTO1 0.655 → TTO3 0.554），第三輪明顯衰退；K% 從 27.8% 掉到 20.8%（Δ -7.0pp）
- 🟠 HOME chain breaks at #1-2：OPS 落差 0.229
- 🔴 AWAY chain breaks at #6-7：OPS 落差 0.442
