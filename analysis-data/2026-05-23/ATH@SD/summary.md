...
### Lucas Giolito (HOME, RHP, 31 📉 初期退化)
...
### J.T. Ginn (AWAY, RHP, 27 ⚡ 巔峰期)
- **Tier 驗證**：腳本 tier_v2 = 🟠 Strong Ace（xFIP p85, K-BB% p69），gap vs ERA-only = +3.4
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 3.26 / 6 / <!-- AI --> | 4.54 / 2 / <!-- AI --> |
...
- ⚠️ HOME 投手 Flag 8 (era_xera_delta=+3.02):
  - <!-- AI 補：是運氣還結構性？是否影響本場判斷？不自動下修預測 -->
- ⚠️ HOME 打線 Flag 3 (last7 BABIP=0.222):
  - <!-- AI 補：可能回歸或可能持續？是否影響本場判斷？不自動 ±run value -->

### 額外信號
- 🟠 HOME single-pitch dependent：主球種使用率 55.1%（≥45.0%）
- 🟠 HOME TTO3 penalty：OPS Δ +-0.098（TTO1 0.811 → TTO3 0.713），第三輪明顯衰退；K% 從 27.0% 掉到 20.5%（Δ -6.5pp）（career fallback）
- 🟠 AWAY TTO3 penalty：OPS Δ +-0.231（TTO1 0.756 → TTO3 0.525），第三輪明顯衰退；K% 從 23.4% 掉到 13.2%（Δ -10.2pp）
- 🟠 HOME chain breaks at #7-8：OPS 落差 0.221
