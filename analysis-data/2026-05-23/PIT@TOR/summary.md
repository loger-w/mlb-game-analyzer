...
- **Tier 驗證**：腳本 tier_v2 = 🟢 Back-end Starter（xFIP p47, K-BB% p30），gap vs ERA-only = -8.3
...
### Paul Skenes (AWAY, RHP, 23 📈 成長期)
- **Tier 驗證**：腳本 tier_v2 = 🔴 Elite Ace（xFIP p95, K-BB% p95），gap vs ERA-only = +8.7
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 3.9 / 9 / <!-- AI --> | 4.21 / 2 / <!-- AI --> |
...
- ⚠️ HOME 投手 Flag 8 (era_xera_delta=-2.16):
  - <!-- AI 補：是運氣還結構性？是否影響本場判斷？不自動下修預測 -->

### 額外信號
- 🟠 HOME TTO3 penalty：OPS Δ +0.065（TTO1 0.825 → TTO3 0.890），第三輪明顯衰退；K% 從 20.5% 掉到 14.1%（Δ -6.4pp）（career fallback）
- 🔴 AWAY TTO3 penalty：OPS Δ +0.297（TTO1 0.472 → TTO3 0.769），第三輪明顯衰退；K% 從 38.4% 掉到 25.6%（Δ -12.8pp）
- 🟠 HOME chain breaks at #8-9：OPS 落差 0.178
- 🔴 AWAY chain breaks at #7-8：OPS 落差 0.349
...
| HOME | 2.3 | <!-- AI 補 --> | <!-- AI 補 --> |
| AWAY | 4.1 | <!-- AI 補 --> | <!-- AI 補 --> |
| Total | 6.3999999999999995 | <!-- AI 補 --> | <!-- AI 補 --> |
