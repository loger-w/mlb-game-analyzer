...
- **Tier 驗證**：腳本 tier_v2 = 🟠 Strong Ace（xFIP p95, K-BB% p64），gap vs ERA-only = +43.4
...
- **Tier 驗證**：腳本 tier_v2 = 🟠 Strong Ace（xFIP p84, K-BB% p75），gap vs ERA-only = -3.1
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 3.61 / 7 / <!-- AI --> | 5.7 / 8 / <!-- AI --> |
...
- ⚠️ HOME 打線 Flag 3 (last7 BABIP=0.236):
  - <!-- AI 補：可能回歸或可能持續？是否影響本場判斷？不自動 ±run value -->
- ⚠️ AWAY 打線 Flag 3 (last7 BABIP=0.201):
  - <!-- AI 補：可能回歸或可能持續？是否影響本場判斷？不自動 ±run value -->

### 額外信號
- 🟠 HOME chain breaks at #5-6：OPS 落差 0.207
- 🟠 AWAY chain breaks at #8-9：OPS 落差 0.293
...
- 天氣：Partly Cloudy, 57°F, wind 9 mph, In From LF
