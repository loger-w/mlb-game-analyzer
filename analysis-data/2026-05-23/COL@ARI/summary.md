...
### Zac Gallen (HOME, RHP, 30 📉 初期退化)
- **Tier 驗證**：腳本 tier_v2 = 🟡 Solid Starter（xFIP p60, K-BB% p41），gap vs ERA-only = +19.0
...
- **Tier 驗證**：腳本 tier_v2 = 🟡 Solid Starter（xFIP p66, K-BB% p35），gap vs ERA-only = +41.3
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 4.2 / 6 / <!-- AI --> | 4.32 / 7 / <!-- AI --> |
...
### 額外信號
- 🟠 HOME reverse platoon Δ +0.092（vs RHB OPS 0.836 > vs LHB OPS 0.744）— RHP 對非預期手別反而吃虧
- 🔴 HOME TTO3 penalty：OPS Δ +0.449（TTO1 0.743 → TTO3 1.192），第三輪明顯衰退；K% 從 15.7% 掉到 9.3%（Δ -6.4pp）
- ℹ️ AWAY balanced 4+ pitches：最高球種僅 20.7%（<25.0%）
- 🔴 AWAY TTO3 penalty：OPS Δ +0.346（TTO1 0.960 → TTO3 1.306），第三輪明顯衰退；K% 從 19.4% 掉到 12.5%（Δ -6.9pp）
- 🟠 HOME chain breaks at #2-3：OPS 落差 0.255
- 🟠 AWAY chain breaks at #4-5：OPS 落差 0.224
...
| HOME | 5.7 | <!-- AI 補 --> | <!-- AI 補 --> |
| AWAY | 4.9 | <!-- AI 補 --> | <!-- AI 補 --> |
| Total | 10.600000000000001 | <!-- AI 補 --> | <!-- AI 補 --> |
