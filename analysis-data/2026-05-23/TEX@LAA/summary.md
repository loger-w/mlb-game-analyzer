...
- **Tier 驗證**：腳本 tier_v2 = 🟡 Solid Starter（xFIP p44, K-BB% p32），gap vs ERA-only = -33.6
...
### Nathan Eovaldi (AWAY, RHP, 36 📉📉 明顯退化)
- **Tier 驗證**：腳本 tier_v2 = 🔴 Elite Ace（xFIP p95, K-BB% p95），gap vs ERA-only = +23.0
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 5.5 / 4 / <!-- AI --> | 2.92 / 4 / <!-- AI --> |
...
- ⚠️ HOME 打線 Flag 3 (last7 BABIP=0.219):
  - <!-- AI 補：可能回歸或可能持續？是否影響本場判斷？不自動 ±run value -->

### 額外信號
- 🔴 HOME reverse platoon Δ +0.204（vs RHB OPS 0.727 > vs LHB OPS 0.523）— RHP 對非預期手別反而吃虧
- 🟠 AWAY TTO3 penalty：OPS Δ +0.032（TTO1 0.770 → TTO3 0.802），第三輪明顯衰退；K% 從 28.3% 掉到 22.2%（Δ -6.1pp）
- 🟠 HOME chain breaks at #6-7：OPS 落差 0.278
- 🟠 AWAY chain breaks at #3-4：OPS 落差 0.222
