...
- **Tier 驗證**：腳本 tier_v2 = 🟢 Back-end Starter（xFIP p33, K-BB% p35），gap vs ERA-only = -16.9
...
### Jake Irvin (AWAY, RHP, 29 ⚡ 巔峰期)
- **Tier 驗證**：腳本 tier_v2 = 🟠 Strong Ace（xFIP p83, K-BB% p75），gap vs ERA-only = +62.9
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 3.14 / 6 / <!-- AI --> | 4.87 / 6 / <!-- AI --> |
...
### 額外信號
- 🟠 HOME TTO3 penalty：OPS Δ +-0.015（TTO1 0.557 → TTO3 0.542），第三輪明顯衰退；K% 從 22.3% 掉到 15.4%（Δ -6.9pp）
- 🟠 AWAY reverse platoon Δ +0.094（vs RHB OPS 0.842 > vs LHB OPS 0.748）— RHP 對非預期手別反而吃虧
- 🟠 HOME chain breaks at #8-9：OPS 落差 0.217
- 🟠 AWAY chain breaks at #3-4：OPS 落差 0.293
