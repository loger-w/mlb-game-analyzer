...
- **Tier 驗證**：腳本 tier_v2 = 🟠 Strong Ace（xFIP p75, K-BB% p66），gap vs ERA-only = +44.2
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 3.22 / 5 / <!-- AI --> | 2.98 / 12 / <!-- AI --> |
...
### 額外信號
- 🟠 HOME TTO3 penalty：OPS Δ +0.078（TTO1 0.816 → TTO3 0.894），第三輪明顯衰退；K% 從 22.4% 掉到 5.9%（Δ -16.5pp）（career fallback）
- 🟠 AWAY TTO3 penalty：OPS Δ +-0.059（TTO1 0.653 → TTO3 0.594），第三輪明顯衰退；K% 從 25.9% 掉到 16.2%（Δ -9.7pp）
- 🔴 HOME chain breaks at #4-5：OPS 落差 0.332
