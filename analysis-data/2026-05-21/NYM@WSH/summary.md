...
### Cade Cavalli (HOME, RHP, 27 ⚡ 巔峰期)
- **Tier 驗證**：腳本 tier_v2 = 🔴 Elite Ace（xFIP p95, K-BB% p80），gap vs ERA-only = +32.9
...
### David Peterson (AWAY, LHP, 30 📉 初期退化)
- **Tier 驗證**：腳本 tier_v2 = 🔴 Elite Ace（xFIP p95, K-BB% p75），gap vs ERA-only = +59.5
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 4.95 / 6 / <!-- AI --> | 3.48 / 7 / <!-- AI --> |
...
### 額外信號
- 🟠 HOME TTO3 penalty：OPS Δ +0.111（TTO1 0.632 → TTO3 0.743），第三輪明顯衰退
- 🔴 HOME chain breaks at #4-5：OPS 落差 0.428
- 🟠 AWAY chain breaks at #3-4：OPS 落差 0.256
...
- 天氣：Cloudy, 62°F, wind 10 mph, In From CF
