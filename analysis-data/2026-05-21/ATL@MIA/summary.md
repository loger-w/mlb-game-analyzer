...
- **Tier 驗證**：腳本 tier_v2 = 🟡 Solid Starter（xFIP p60, K-BB% p45），gap vs ERA-only = -7.4
...
### Spencer Strider (AWAY, RHP, 27 ⚡ 巔峰期)
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 3.38 / 3 / <!-- AI --> | 3.18 / 6 / <!-- AI --> |
...
- 🔴 HOME TTO3 penalty：OPS Δ +0.282（TTO1 0.501 → TTO3 0.783），第三輪明顯衰退；K% 從 22.5% 掉到 15.6%（Δ -6.9pp）
- 🟠 AWAY single-pitch dependent：主球種使用率 48.5%（≥45.0%）
- 🟠 AWAY TTO3 penalty：OPS Δ +0.085（TTO1 0.611 → TTO3 0.696），第三輪明顯衰退；K% 從 36.1% 掉到 30.3%（Δ -5.8pp）（career fallback）
- 🟠 HOME chain breaks at #1-2：OPS 落差 0.220
- 🟠 AWAY chain breaks at #8-9：OPS 落差 0.216
