...
### José Soriano (HOME, RHP, 27 ⚡ 巔峰期)
- **Tier 驗證**：腳本 tier_v2 = 🔴 Elite Ace（xFIP p95, K-BB% p85），gap vs ERA-only = +1.6
...
- **Tier 驗證**：腳本 tier_v2 = 🟡 Solid Starter（xFIP p63, K-BB% p46），gap vs ERA-only = +12.0
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 5.6 / 4 / <!-- AI --> | 4.51 / 2 / <!-- AI --> |
...
- ⚠️ HOME 投手 Flag 8 (era_xera_delta=-1.59):
  - <!-- AI 補：是運氣還結構性？是否影響本場判斷？不自動下修預測 -->
- ⚠️ HOME 打線 Flag 3 (last7 BABIP=0.234):
  - <!-- AI 補：可能回歸或可能持續？是否影響本場判斷？不自動 ±run value -->

### 額外信號
- 🟠 HOME TTO3 penalty：OPS Δ +0.119（TTO1 0.493 → TTO3 0.612），第三輪明顯衰退
- ℹ️ AWAY balanced 4+ pitches：最高球種僅 23.9%（<25.0%）
- 🟠 AWAY TTO3 penalty：OPS Δ +-0.290（TTO1 0.893 → TTO3 0.603），第三輪明顯衰退；K% 從 22.2% 掉到 13.6%（Δ -8.6pp）
- 🟠 HOME chain breaks at #6-7：OPS 落差 0.228
- 🔴 AWAY chain breaks at #2-3：OPS 落差 0.325
