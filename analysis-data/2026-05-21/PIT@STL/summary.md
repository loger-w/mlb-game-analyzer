...
- **Tier 驗證**：腳本 tier_v2 = 🟡 Solid Starter（xFIP p64, K-BB% p54），gap vs ERA-only = +26.9
...
- **Tier 驗證**：腳本 tier_v2 = 🔴 Elite Ace（xFIP p95, K-BB% p95），gap vs ERA-only = +16.4
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 4.53 / 0 / <!-- AI --> | 4.13 / 2 / <!-- AI --> |
...
### 額外信號
- 🟠 HOME TTO3 penalty：OPS Δ +-0.137（TTO1 0.792 → TTO3 0.655），第三輪明顯衰退；K% 從 19.8% 掉到 13.0%（Δ -6.8pp）
- 🟠 AWAY TTO3 penalty：OPS Δ +0.082（TTO1 0.544 → TTO3 0.626），第三輪明顯衰退；K% 從 29.6% 掉到 23.2%（Δ -6.4pp）
- 🟠 HOME chain breaks at #4-5：OPS 落差 0.241
- 🟠 AWAY chain breaks at #5-6：OPS 落差 0.158
  - <!-- AI 補：本場是否受此信號影響？是否與 Flag 3/8 雙重壓力 → 1-2 句敘事 -->

## 條件修正

- Park Factor: 98.0 → -0.10 run
- 天氣：Partly Cloudy, 67°F, wind 8 mph, L To R
