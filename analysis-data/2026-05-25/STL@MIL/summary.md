...
- **Tier 驗證**：腳本 tier_v2 = 🟡 Solid Starter（xFIP p49, K-BB% p51），gap vs ERA-only = +18.4
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 3.43 / 5 / <!-- AI --> | 4.39 / 0 / <!-- AI --> |
...
- ⚠️ HOME 打線 Flag 3 (last7 BABIP=0.258):
  - <!-- AI 補：可能回歸或可能持續？是否影響本場判斷？不自動 ±run value -->

### 額外信號
- 🟠 HOME reverse platoon Δ +0.127（vs RHB OPS 0.544 > vs LHB OPS 0.417）— RHP 對非預期手別反而吃虧
- 🟠 HOME single-pitch dependent：主球種使用率 60.8%（≥45.0%）
- 🟠 HOME TTO3 penalty：OPS Δ +-0.026（TTO1 0.468 → TTO3 0.442），第三輪明顯衰退；K% 從 43.4% 掉到 28.3%（Δ -15.1pp）
- 🔴 AWAY reverse platoon Δ +0.400（vs LHB OPS 1.125 > vs RHB OPS 0.725）— LHP 對非預期手別反而吃虧
- 🔴 AWAY TTO3 penalty：OPS Δ +0.689（TTO1 0.659 → TTO3 1.348），第三輪明顯衰退；K% 從 28.3% 掉到 8.5%（Δ -19.8pp）
- 🔴 HOME chain breaks at #5-6：OPS 落差 0.332
- 🔴 AWAY chain breaks at #7-8：OPS 落差 0.639
...
- 天氣：Partly Cloudy, 80°F, wind 10 mph, In From RF
...
| HOME | 5.1 | <!-- AI 補 --> | <!-- AI 補 --> |
| AWAY | 2.1 | <!-- AI 補 --> | <!-- AI 補 --> |
| Total | 7.199999999999999 | <!-- AI 補 --> | <!-- AI 補 --> |
