...
- **Tier 驗證**：腳本 tier_v2 = 🟠 Strong Ace（xFIP p95, K-BB% p68），gap vs ERA-only = +28.5
...
- **Tier 驗證**：腳本 tier_v2 = 🔴 Elite Ace（xFIP p87, K-BB% p83），gap vs ERA-only = +3.7
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
### AWAY — season tier 🟢 Weak / heat 🔥 Hot
- 打線來源：🟢 official
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 4.66 / 7 / <!-- AI --> | 4.38 / 6 / <!-- AI --> |
...
- ⚠️ HOME 打線 Flag 3 (last7 BABIP=0.257):
  - <!-- AI 補：可能回歸或可能持續？是否影響本場判斷？不自動 ±run value -->

### 額外信號
- 🔴 HOME reverse platoon Δ +0.258（vs RHB OPS 0.948 > vs LHB OPS 0.690）— RHP 對非預期手別反而吃虧
- 🟠 HOME TTO3 penalty：OPS Δ +0.118（TTO1 0.729 → TTO3 0.847），第三輪明顯衰退
- 🟠 AWAY TTO3 penalty：OPS Δ +0.010（TTO1 0.577 → TTO3 0.587），第三輪明顯衰退；K% 從 31.3% 掉到 23.6%（Δ -7.7pp）（career fallback）
- 🟠 HOME chain breaks at #8-9：OPS 落差 0.276
- 🔴 AWAY chain breaks at #6-7：OPS 落差 0.540
...
- 天氣：Overcast, 68°F, wind 3 mph, R To L
