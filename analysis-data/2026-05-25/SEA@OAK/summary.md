...
- **Tier 驗證**：腳本 tier_v2 = 🟢 Back-end Starter（xFIP p30, K-BB% p45），gap vs ERA-only = -24.7
...
- **Tier 驗證**：腳本 tier_v2 = 🟠 Strong Ace（xFIP p81, K-BB% p71），gap vs ERA-only = +61.7
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 4.33 / 2 / <!-- AI --> | 3.16 / 3 / <!-- AI --> |
...
- ⚠️ HOME 投手 Flag 8 (era_xera_delta=-1.60):
  - <!-- AI 補：是運氣還結構性？是否影響本場判斷？不自動下修預測 -->
- ⚠️ AWAY 打線 Flag 3 (last7 BABIP=0.199):
  - <!-- AI 補：可能回歸或可能持續？是否影響本場判斷？不自動 ±run value -->

### 額外信號
- 🟠 HOME TTO3 penalty：OPS Δ +-0.003（TTO1 0.819 → TTO3 0.816），第三輪明顯衰退；K% 從 24.2% 掉到 11.4%（Δ -12.8pp）
- 🟠 AWAY TTO3 penalty：OPS Δ +-0.046（TTO1 0.741 → TTO3 0.695），第三輪明顯衰退；K% 從 24.5% 掉到 19.5%（Δ -5.0pp）
- 🟠 HOME chain breaks at #7-8：OPS 落差 0.268
