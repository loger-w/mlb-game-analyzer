...
### Jesús Luzardo (AWAY, LHP, 28 ⚡ 巔峰期)
- **Tier 驗證**：腳本 tier_v2 = 🔴 Elite Ace（xFIP p95, K-BB% p95），gap vs ERA-only = +56.4
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 3.16 / 6 / <!-- AI --> | 3.99 / 3 / <!-- AI --> |
...
- ⚠️ HOME 投手 Flag 8 (era_xera_delta=-2.49):
  - <!-- AI 補：是運氣還結構性？是否影響本場判斷？不自動下修預測 -->
- ⚠️ AWAY 投手 Flag 8 (era_xera_delta=+1.64):
  - <!-- AI 補：是運氣還結構性？是否影響本場判斷？不自動下修預測 -->
- ⚠️ HOME 打線 Flag 3 (last7 BABIP=0.188):
...
- 🔴 AWAY TTO3 penalty：OPS Δ +0.552（TTO1 0.418 → TTO3 0.970），第三輪明顯衰退；K% 從 31.7% 掉到 21.7%（Δ -10.0pp）
- 🟠 HOME chain breaks at #7-8：OPS 落差 0.213
- 🟠 AWAY chain breaks at #3-4：OPS 落差 0.281
