...
- **Tier 驗證**：腳本 tier_v2 = 🟠 Strong Ace（xFIP p75, K-BB% p59），gap vs ERA-only = +38.9
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 3.85 / 9 / <!-- AI --> | 3.33 / 3 / <!-- AI --> |
...
### 額外信號
- 🟠 HOME single-pitch dependent：主球種使用率 47.7%（≥45.0%）
- 🔴 HOME TTO3 penalty：OPS Δ +0.270（TTO1 0.605 → TTO3 0.875），第三輪明顯衰退；K% 從 31.2% 掉到 21.9%（Δ -9.3pp）（career fallback）
- 🟠 HOME chain breaks at #7-8：OPS 落差 0.178
- 🟠 AWAY chain breaks at #1-2：OPS 落差 0.248
