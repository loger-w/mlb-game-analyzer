...
- **Tier 驗證**：腳本 tier_v2 = ⚪ Below Average（xFIP p5, K-BB% p5），gap vs ERA-only = -40.7
...
- **Tier 驗證**：腳本 tier_v2 = 🟠 Strong Ace（xFIP p72, K-BB% p78），gap vs ERA-only = +33.2
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 4.22 / 6 / <!-- AI --> | 3.03 / 6 / <!-- AI --> |
...
### 額外信號
- 🔴 HOME reverse platoon Δ +0.313（vs LHB OPS 1.026 > vs RHB OPS 0.713）— LHP 對非預期手別反而吃虧
- 🟠 AWAY reverse platoon Δ +0.133（vs LHB OPS 0.774 > vs RHB OPS 0.641）— LHP 對非預期手別反而吃虧
- 🔴 AWAY TTO3 penalty：OPS Δ +0.298（TTO1 0.647 → TTO3 0.945），第三輪明顯衰退；K% 從 31.1% 掉到 23.8%（Δ -7.3pp）
- 🟠 HOME chain breaks at #2-3：OPS 落差 0.287
- 🟠 AWAY chain breaks at #4-5：OPS 落差 0.217
