...
### Framber Valdez (HOME, LHP, 32 📉 初期退化)
- **Tier 驗證**：腳本 tier_v2 = 🟠 Strong Ace（xFIP p82, K-BB% p57），gap vs ERA-only = +21.0
...
- **Tier 驗證**：腳本 tier_v2 = 🟡 Solid Starter（xFIP p64, K-BB% p51），gap vs ERA-only = +44.1
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 3.84 / 8 / <!-- AI --> | 4.0 / 1 / <!-- AI --> |
...
### 額外信號
- 🟠 HOME single-pitch dependent：主球種使用率 46.1%（≥45.0%）
- 🔴 AWAY reverse platoon Δ +0.235（vs RHB OPS 0.961 > vs LHB OPS 0.726）— RHP 對非預期手別反而吃虧
- 🔴 AWAY TTO3 penalty：OPS Δ +0.734（TTO1 0.688 → TTO3 1.422），第三輪明顯衰退；K% 從 18.7% 掉到 9.5%（Δ -9.2pp）
- 🟠 HOME chain breaks at #4-5：OPS 落差 0.195
- 🟠 AWAY chain breaks at #6-7：OPS 落差 0.174
...
- 天氣：Partly Cloudy, 76°F, wind 21 mph, Out To LF
