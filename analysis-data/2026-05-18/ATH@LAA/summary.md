...
- **Tier 驗證**：腳本 tier_v2 = 🟠 Strong Ace（xFIP p72, K-BB% p51），gap vs ERA-only = -5.6
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 5.47 / 4 / <!-- AI --> | 4.68 / 2 / <!-- AI --> |
...
### 額外信號
- 🔴 HOME reverse platoon Δ +0.329（vs RHB OPS 0.832 > vs LHB OPS 0.503）— RHP 對非預期手別反而吃虧
- 🟠 AWAY TTO3 penalty：OPS Δ +-0.079（TTO1 0.824 → TTO3 0.745），第三輪明顯衰退；K% 從 25.2% 掉到 14.3%（Δ -10.9pp）（career fallback）
- 🟠 HOME chain breaks at #2-3：OPS 落差 0.179
- 🔴 AWAY chain breaks at #2-3：OPS 落差 0.340
