...
- **Tier 驗證**：腳本 tier_v2 = 🟡 Solid Starter（xFIP p70, K-BB% p63），gap vs ERA-only = +4.0
...
- **Tier 驗證**：腳本 tier_v2 = 🟡 Solid Starter（xFIP p68, K-BB% p45），gap vs ERA-only = -15.0
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 4.42 / 6 / <!-- AI --> | 3.16 / 6 / <!-- AI --> |
...
- ⚠️ AWAY 投手 Flag 8 (era_xera_delta=-1.98):
  - <!-- AI 補：是運氣還結構性？是否影響本場判斷？不自動下修預測 -->
- ⚠️ AWAY 打線 Flag 3 (last7 BABIP=0.248):
  - <!-- AI 補：可能回歸或可能持續？是否影響本場判斷？不自動 ±run value -->

### 額外信號
- ℹ️ HOME balanced 4+ pitches：最高球種僅 20.0%（<25.0%）
- 🟠 HOME TTO3 penalty：OPS Δ +-0.073（TTO1 0.801 → TTO3 0.728），第三輪明顯衰退；K% 從 21.5% 掉到 16.7%（Δ -4.8pp）
- 🔴 AWAY reverse platoon Δ +0.269（vs RHB OPS 0.822 > vs LHB OPS 0.553）— RHP 對非預期手別反而吃虧
- ℹ️ AWAY balanced 4+ pitches：最高球種僅 22.9%（<25.0%）
- 🔴 AWAY TTO3 penalty：OPS Δ +0.184（TTO1 0.600 → TTO3 0.784），第三輪明顯衰退；K% 從 27.5% 掉到 22.4%（Δ -5.1pp）（career fallback）
- 🟠 HOME chain breaks at #2-3：OPS 落差 0.244
- 🟠 AWAY chain breaks at #4-5：OPS 落差 0.294
...
- 天氣：Partly Cloudy, 81°F, wind 18 mph, Out To LF
