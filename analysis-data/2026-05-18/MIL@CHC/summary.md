...
- **Tier 驗證**：腳本 tier_v2 = 🔴 Elite Ace（xFIP p95, K-BB% p95），gap vs ERA-only = +0.0
...
- **Tier 驗證**：腳本 tier_v2 = 🟡 Solid Starter（xFIP p61, K-BB% p52），gap vs ERA-only = +46.6
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 3.8 / 8 / <!-- AI --> | 3.36 / 5 / <!-- AI --> |
...
### 額外信號
- 🟠 HOME reverse platoon Δ +0.152（vs LHB OPS 0.654 > vs RHB OPS 0.502）— LHP 對非預期手別反而吃虧
- 🔴 HOME TTO3 penalty：OPS Δ +0.261（TTO1 0.585 → TTO3 0.846），第三輪明顯衰退；K% 從 30.8% 掉到 11.5%（Δ -19.3pp）
- 🟠 AWAY chain breaks at #5-6：OPS 落差 0.164
