...
### Shane McClanahan (HOME, LHP, 29 ⚡ 巔峰期)
- **Tier 驗證**：腳本 tier_v2 = 🔴 Elite Ace（xFIP p87, K-BB% p82），gap vs ERA-only = -5.7
...
### Trevor Rogers (AWAY, LHP, 28 ⚡ 巔峰期)
- **Tier 驗證**：腳本 tier_v2 = 🟡 Solid Starter（xFIP p64, K-BB% p56），gap vs ERA-only = +48.6
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 4.44 / 7 / <!-- AI --> | 4.38 / 8 / <!-- AI --> |
...
- ⚠️ AWAY 投手 Flag 8 (era_xera_delta=+1.64):
  - <!-- AI 補：是運氣還結構性？是否影響本場判斷？不自動下修預測 -->

### 額外信號
- 🟠 HOME TTO3 penalty：OPS Δ +0.002（TTO1 0.579 → TTO3 0.581），第三輪明顯衰退；K% 從 31.4% 掉到 23.3%（Δ -8.1pp）（career fallback）
- 🟠 AWAY TTO3 penalty：OPS Δ +0.134（TTO1 0.571 → TTO3 0.705），第三輪明顯衰退；K% 從 27.0% 掉到 20.0%（Δ -7.0pp）
- 🟠 HOME chain breaks at #3-4：OPS 落差 0.157
- 🟠 AWAY chain breaks at #8-9：OPS 落差 0.241
