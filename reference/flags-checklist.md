...
- 處理：腳本失敗 → 向使用者回報，禁止改走 WebSearch / 記憶。詳見 `SKILL.md` §初始化「模式切換規範」
...
- 處理：B9 雙向閘門。詳見 `SKILL.md` §Phase 3
...
- 處理：predict.py 會 reject。詳見 `SKILL.md` §Phase 3 §3.5 phase3_summary.md 存檔
...
- 處理：predict.py 會 reject。詳見 `SKILL.md` §Phase 4

### 10. shell redirect `>` 取代 --output / -o
- 觸發：腳本呼叫用 `>` 寫檔
- 處理：所有腳本必須用 `--output` / `-o`。詳見 `SKILL.md` §初始化「模式切換規範」
...
每條規則均可透過 Phase 閘門自檢。完整 Phase 順序見 `SKILL.md`；觸發時的補救動作見對應 Phase section 與 cross-ref 檔。
