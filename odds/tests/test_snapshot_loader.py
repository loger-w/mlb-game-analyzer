...
def test_loaded_snapshot_has_tw_field():
    """每份 Snapshot 應有 snapshot_time_tw（TW wall clock）。"""
    snapshots = load_snapshots_for_et_date("2026-04-27", FIXTURES)
    for s in snapshots:
        assert s.snapshot_time_tw is not None
        # snapshot_time_tw = snapshot_time_utc + 8h
        expected_tw = (s.snapshot_time_utc.astimezone(TW)).replace(tzinfo=None)
        assert s.snapshot_time_tw == expected_tw


def test_collect_timeline_missing_date_returns_empty():
    """不存在的 game_date_tw → collect_game_timeline 回空 dict。"""
...
def test_collect_timeline_groups_by_game_key():
    ...
    Fixture 04-27 ET 場次（commence_utc 2026-04-27T22:11Z 等）在 TW 對應日期為 2026-04-28。
    """
    snapshots = load_snapshots_for_et_date("2026-04-27", FIXTURES)
...
def test_collect_timeline_filters_by_game_date_tw():
    """commence_utc → game_date_tw 不符的場應被跳過。"""
    s27 = load_snapshots_for_et_date("2026-04-27", FIXTURES)
    s26 = load_snapshots_for_et_date("2026-04-26", FIXTURES)
    all_snaps = s27 + s26
    # 只要 TW 4/28 的場（= ET 4/27 開打）；ET 4/26 開打的 Detroit 在 TW 4/27，不該出現
    timelines = collect_game_timeline(all_snaps, "2026-04-28")
    keys = list(timelines.keys())
    assert all(k[0] != "Detroit Tigers" for k in keys)


def test_cross_day_snapshot_contributes_to_later_date():
    """檔名為 04-28 的 snapshot 內含 ET 4/29 場次（TBR@CLE, commence_utc 2026-04-29T17:11Z），
    在 TW 對應日期 2026-04-30 應能取到那場 timeline。
    """
    snapshots = load_snapshots_for_et_date("2026-04-30", FIXTURES)
    timelines = collect_game_timeline(snapshots, "2026-04-30")
    keys = list(timelines.keys())
    assert any(k[0] == "Tampa Bay Rays" and k[1] == "Cleveland Guardians" for k in keys), \
        f"TW 4/30 TBR@CLE timeline 應出現，實際 keys={keys}"
    # 同時驗證 ET 4/28 的 Houston 場（commence_utc 2026-04-28T22:38Z = TW 4/29 06:38）不該誤入 TW 4/30 timeline
    assert not any(k[0] == "Houston Astros" for k in keys), \
        "ET 4/28 HOU@BAL（TW 4/29）不該出現在 TW 4/30 timeline"


def test_early_et_game_groups_to_same_tw_day():
    """ET 早場（commence_utc UTC 13:30 = ET 09:30 = TW 同日 21:30）→ game_date_tw = 同日。

    驗證方案 2 的核心優勢：分組基準直接從 commence_utc 算 TW，London Series 等早場
    不會錯分到「TW 隔日」。
    """
    g = _make_game(
        away="London A", home="London B",
        commence_utc="2026-04-29T13:30:00Z",     # ET 09:30，TW 21:30 同日
...
def test_doubleheader_separate_keys():
    ...
    commence_utc 2026-05-01T17:05Z = ET 13:05 = TW 5/2 01:05 → game_date_tw = 2026-05-02
    """
    ...
    timelines = collect_game_timeline([snap1, snap2, snap3], "2026-05-02")
...
def test_team_name_mismatch_skips_game():
    ...
    timelines = collect_game_timeline([snap], "2026-05-02")
    assert timelines == {}
...
def test_post_commence_snapshot_skipped():
    ...
    snap_after = Snapshot(
        ...
        snapshot_time_tw=datetime(2026, 5, 2, 2, 5),
        games=[g],
    )
    snap_before = Snapshot(
        snapshot_time_et=datetime(2026, 5, 1, 8, 0),
        snapshot_time_utc=datetime(2026, 5, 1, 12, 0, tzinfo=timezone.utc),
        snapshot_time_tw=datetime(2026, 5, 1, 20, 0),
        games=[g],
    )
    timelines = collect_game_timeline([snap_before, snap_after], "2026-05-02")
    assert len(timelines) == 1
...
def test_game_record_has_tw_fields():
    """每筆 GameRecord 應有 4 個 TW 欄位，格式正確。"""
    snapshots = load_snapshots_for_et_date("2026-04-27", FIXTURES)
    timelines = collect_game_timeline(snapshots, "2026-04-28")
    record = list(timelines.values())[0][0]
    assert record.commence_tw_label.endswith(" TW")
    assert re.match(r"^\d{4}-\d{2}-\d{2} \d{2}:\d{2} TW$", record.commence_tw_label)
    assert re.match(r"^\d{2}-\d{2} \d{2}:\d{2}$", record.snapshot_time_tw_label)
    assert re.match(r"^\d{4}-\d{2}-\d{2}$", record.game_date_tw)
    assert record.game_date_tw == "2026-04-28"
    assert record.snapshot_time_tw is not None
...
def test_select_anchor_single_record():
    ...
    snapshots[1] = 04-27_00-00-ET（第一個含 ET 4/27 場次的 snapshot）；
    這些場次 commence ET 4/27 → game_date_tw = 2026-04-28。
    """
    snapshots = load_snapshots_for_et_date("2026-04-27", FIXTURES)
...
def _make_synthetic_snapshot(snapshot_time_et_label: str, games: list[dict]) -> Snapshot:
    ...
    snap_utc = et_dt.replace(tzinfo=timezone.utc)   # 簡化測試：用 UTC tz 物件代替（時區數值對排序仍正確）
