...
- **Tier 驗證**：腳本 tier_v2 = 🔴 Elite Ace（xFIP p95, K-BB% p87），gap vs ERA-only = +19.8
...
- **Tier 驗證**：腳本 tier_v2 = ⚪ Below Average（xFIP p14, K-BB% p18），gap vs ERA-only = +6.9
  - <!-- AI 補：是否同意 score-derived tier？若 |gap| ≥ 15 → 簡述運氣 vs 結構性，不自動下修預測 -->
- **Reverse platoon 信號**：見 dossier `## 🎯 訊號摘要`（若 fired）
  - <!-- AI 補：若 fired，本場對手核心打者手別組成是否放大此風險？ -->
- **對手打線威脅**：<!-- AI 補：基於 dossier 投手對決表 + 上述兩信號 -->
...
| | HOME | AWAY |
|---|---|---|
| ERA / 投手 IL / 核心 IL 估計 | 3.29 / 7 / <!-- AI --> | 4.14 / 6 / <!-- AI --> |
...
- ⚠️ AWAY 投手 Flag 8 (era_xera_delta=-2.50):
  - <!-- AI 補：是運氣還結構性？是否影響本場判斷？不自動下修預測 -->

### 額外信號
- 🟠 HOME reverse platoon Δ +0.157（vs RHB OPS 0.680 > vs LHB OPS 0.523）— RHP 對非預期手別反而吃虧
- 🟠 HOME TTO3 penalty：OPS Δ +-0.074（TTO1 0.667 → TTO3 0.593），第三輪明顯衰退；K% 從 28.9% 掉到 20.5%（Δ -8.4pp）
- 🔴 HOME chain breaks at #8-9：OPS 落差 0.337
- 🟠 AWAY chain breaks at #2-3：OPS 落差 0.298
